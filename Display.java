...
public class Display extends CultimaPanel
{
   ...
   public static Font readable, readableP, handWriten, oldEngl, oldEnglBold, oldEnglTitle;
     
   //will readjust the fonts incase the size changes or the client wants more readable fonts   
   public static void loadFonts()
   { 
      if(simpleFonts)
      { 
         String fontName = "Serif";//"Gloucester MT Extra Condensed";
         readable = new Font(fontName, Font.PLAIN, SIZE);
         readableP = new Font(fontName, Font.PLAIN, SIZE);
         handWriten = new Font(fontName, Font.PLAIN, SIZE);
         oldEngl = new Font(fontName, Font.PLAIN, SIZE);
         oldEnglBold = new Font(fontName, Font.PLAIN, SIZE);
         oldEnglTitle = new Font(fontName, Font.BOLD, SIZE*4);
      }
      else
      {
         readable = new Font("Serif", Font.BOLD | Font.ITALIC, SIZE);
         readableP = new Font("Serif", Font.PLAIN | Font.ITALIC, SIZE);
         handWriten = new Font("Pristina", Font.BOLD | Font.ITALIC, SIZE);
         oldEngl = new Font("Old English Text MT", Font.PLAIN, SIZE);
         oldEnglBold = new Font("Old English Text MT", Font.BOLD, SIZE);
         oldEnglTitle = new Font("Old English Text MT", Font.BOLD, SIZE*4);
      }
   }   
      
   public static void drawRainbow(Graphics g, int alpha)	
   {
      if((menuMode==TRAVEL && CultimaPanel.rainbowAlpha==0) || player.getLocationType().contains("pacrealm") || player.getLocationType().contains("frogrealm"))
         return;
   ...
   {
      if(menuMode!=TRAVEL || fogAlpha == 0 || player.getLocationType().contains("pacrealm") || player.getLocationType().contains("frogrealm"))
         return;  
   ...
   public static void drawPrecipitation(Graphics g)
   {
      ...
      if(menuMode == TRAVEL && weather > 0 && !locationType.contains("sewer") && !locationType.contains("cave") && !locationType.contains("mine") && !locationType.contains("dungeon") && !locationType.contains("lair") && !locationType.contains("pacrealm") && !locationType.contains("frogrealm"))
      {
         int numRows = SCREEN_SIZE;
         int numCols = SCREEN_SIZE;
   ...
   {  //allow for thicker lines
      ...
      {
         g2.setColor(new Color(Utilities.rollDie(200,255),Utilities.rollDie(200,255),Utilities.rollDie(230,255)));
         g2.setStroke(new BasicStroke(thickness));
         if(i%4 !=0)
         {
            thickness = Math.max(2, thickness-1);
         }
         Coord node = lightningNodes.get(i);
         int fromX = node.getRow();
         int fromY = node.getCol();
         Coord next = lightningNodes.get(i+1);
         int toX = next.getRow();
         int toY = next.getCol();
   ...
   public static void showAllGraphics(Graphics g)	
   {
      ...
      int x =0, y=0;		      //upper left corner location of where image will be drawn
      float tempTime = time; //change tempTime to some kind of dark if we are in a cave, mine, lair (indoors)
      ...
      {  //find the distance between player position in the cave and the cave entrance to set light level (tempTime)
         ...
         if(!locationType.contains("sewer") && mapIndex>=0 && mapIndex<allLocations.size())
         {
            ...
            {  //brightest when tempTime is 12, darkest when it is zero
               found = true;
               float maxDist = (float)(Math.sqrt(Math.pow(currMap.length,2) + Math.pow(currMap[0].length/2,2)));
               float distFromEntrance = distanceSimple(player.getRow(), player.getCol(), entranceRow, entranceCol);
               float percDist = Math.max(0,Math.min(1, 1 - (distFromEntrance/maxDist)));
               float distFrom12 = Math.max(0, Math.min(12, 12 - (Math.abs(12 - time))));
      ...
      else if(locationType.contains("pacrealm") || locationType.contains("frogrealm"))
      {
         tempTime = 12;
      }
   ...
   public static void drawTraveler(Graphics g, Terrain currentPos)
   {
      ...
      if(flight==null && !sailing)
      {
         ...
         {     
            float alpha = distFromDeadWinter();
            ...
            {         
               ...
               if(isWinter() && !locationType.contains("underworld") && !locationType.contains("frogrealm"))
               {
   ...
   {   
      ...
      else if(menuMode == MAIN)
      {
         ...
         if(!CultimaPanel.soundOn)
            soundState = "off";
         String fontState = "ON";
         if(!CultimaPanel.simpleFonts)
            fontState = "off";   
         g.drawString("(T)oggle sound: "+soundState, x, y);
         y += SIZE;
         g.drawString("(F)ont change: "+fontState, x, y);
         y += SIZE;
      ...
      else if(menuMode == MAP)
      {
         ...
         else
         {
            ...
            if(player.getLocationType().toLowerCase().contains("underworld"))
               ...
               where = "The Pac-Realm";
            else if(player.getLocationType().toLowerCase().contains("frogrealm"))
               where = "The Frog-Realm";
   ...
   public static Coord travelText(Graphics g, int x, int y, Terrain currentPos)
   {
      ...
      else 
      {
         ...
         xAdd += (SIZE*5);
         boolean hasPick = (player.hasItem("lockpick") || player.hasItem("magicpick"));
         boolean nextToLock = (Utilities.nextToALock(map.get(mapIndex), player.getRow(), player.getCol())
            || (!Utilities.nextToALockOrTrap(map.get(mapIndex), player.getRow(), player.getCol()).equals("none") && awareness==4));  //don't show pick option for traps unless we have focus spell
         if(flight==null && !player.isWolfForm() && hasPick && nextToLock)
         {
            g.setColor(new Color(150, 0, 150));
         ...
         if(Utilities.canBreakWall() || CultimaPanel.player.canBashGround())
         {
            g.setColor(new Color(150, 0, 0));
         ...
         {//only have attack as an option if NPC is in range of the current weapon
            g.setColor(new Color(150, 0, 150));
            if(NPC.isHorse(feedable) || feedable.getCharIndex()==NPC.PANGOLIN)
            ...
            if(NPCinView != null && NPCinView.getMapIndex()==player.getMapIndex())
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     else if(NPCinView.getLocationType().contains("pacrealm") || NPCinView.getLocationType().contains("frogrealm"))
                     {
                        g.drawString(NPCinView.getName(), x, y);
                     }
                     else if(NPCinView.isWerewolf())                       //werewolf tag
   ...
   public static Coord showPlayerInfo(Graphics g)
   {
      ...
      g.setFont(readable);
      g.drawString("                    " + player.getAwarenessName(), x, y);
      y += SIZE;
      ...
      g.setFont(readable);
      g.drawString("               " + weapName +weaponAugment, x, y);
      y += SIZE;
      g.setFont(oldEngl);
      if(minDam == maxDam)
         g.drawString("Damage: " + maxDam, x, y);
      else
         g.drawString("Damage: " + minDam+" - "+maxDam, x, y);
      g.setFont(readable);
      ...
      g.setFont(readable);
      g.drawString("               " + weap.getEffect(), x, y);
      g.setFont(oldEngl);
      ...
      g.setFont(readable);
      g.drawString("               " + armor.getName() + ", protection:" + player.getArmorPoints(), x, y);
      ...
      else
      {
         ...
         g.drawString("Items:", x+(SIZE*10), y);
      }
      y += SIZE;
      ...
      if(player.numPotions() > 0)
      {
         ...
         if(potion != null)
         {
            g.drawString("Potion:     ", x, y);
            g.setFont(readable);
            g.drawString("                  " + potion.getName()+"("+player.countPotion(potion.getName())+")", x, y);
         }
      }
      y += SIZE;
      g.setFont(oldEngl);
      if(player.getSpells().size() > 0 && player.getSpellInfo()!=null)
      {
         ...
         if(player.getSpell()==null)         //write spell name in red if we can't cast it
         {
            ...
            g.setColor(new Color(150,0,0));
            g.drawString("Spell:       ", x, y);
            g.setFont(readable);
            g.drawString("                  " + spell.getName()+timeLeft, x, y);
         }
         else
         {
            g.drawString("Spell:       ", x, y);
            g.setFont(readable);
            g.drawString("                  " + spell.getName(), x, y);
         }
      }
      y += SIZE;
      g.setFont(oldEngl);
   ...
   //display terrain, map items, npcs, corpses + effects like sun reflections, shadows and glowing
   public static Coord showGameScreen(Graphics g, int x, int y, float tempTime,  String locationType, Spell timestop)
   {
      int size = SIZE;
      int numRows = SCREEN_SIZE;
      int numCols = SCREEN_SIZE;
      ...
      if(numFrames > lightningFrame + Math.max(20,animDelay))
      {
      ...
      for(int i=0; i<numRows; i++)
      {
         ...
         for(int j=0; j<numCols; j++)
         {
            ...
            float distToDeath = -1;   //the Death character is shrouded in a radius of darkness
            ...
            {  //color darkened cells differently if we sense something is there (depending on awareness)
               ...
               {
                  float d1dist = Display.distanceSimple(CultimaPanel.door1.getRow(),CultimaPanel.door1.getCol(),player.getRow(),player.getCol());
                  float d2dist = Display.distanceSimple(CultimaPanel.door2.getRow(),CultimaPanel.door2.getCol(),player.getRow(),player.getCol());
                  float d3dist = Display.distanceSimple(CultimaPanel.door3.getRow(),CultimaPanel.door3.getCol(),player.getRow(),player.getCol());
               ...
               if((map.get(mapIndex))[r][c]!=0)
               {
                  ...
                  if(CultimaPanel.player.getCurrentTerrain()!=0 && CultimaPanel.player.getRow()==r && CultimaPanel.player.getCol()==c)
                  {
                     ter = allTerrain.get(CultimaPanel.player.getCurrentTerrain());
                  }
                  if(menuMode == MAP)
                  {  //show map elements visited/seen or show all if they have eagle-eye
                     if((map.get(mapIndex))[r][c] < 0 || awareness==4)  //negative values means that we have visited there
                     {
                        ...
                        {//don't show elements close to magic doors (3-doors puzzle)
                           float d1dist = Display.distanceSimple(CultimaPanel.door1.getRow(),CultimaPanel.door1.getCol(),r,c);
                           float d2dist = Display.distanceSimple(CultimaPanel.door2.getRow(),CultimaPanel.door2.getCol(),r,c);
                           float d3dist = Display.distanceSimple(CultimaPanel.door3.getRow(),CultimaPanel.door3.getCol(),r,c);
                  ...
                  else  //in Travel mode
                  {
                     ...
                     else
                     {
                        ...
                        if(isWinter() && ter.getName().toLowerCase().contains("water") && !locationType.contains("underworld") && !locationType.contains("frogrealm"))
                  ...
                  if(lightningNodes.size() > 0 && numFrames <= lightningFrame+Math.max(20,animDelay))
                  {
                     drawLightning(g);
                  }
               }
      ...
      if(menuMode == TRAVEL)
      {
         for(int i=0; i<corpsesToDraw.size(); i++)
         {
            ...
            if(cp.getHitTime() >= CultimaPanel.numFrames && !player.getWeapon().getName().toLowerCase().contains("lightning"))
            {
            ...
            {
               int dist = 0;
               float sizeMod = (float)Math.min(2,cp.getSize()) * 0.8f;
               float sizeModWater = Math.max(1, sizeMod);
               if(mapIndex == 0)
         ...
         for(Coord npcCoord: npcsToDraw.keySet())
         {
            ...
            for(NPCPlayer npc: npcsThere)
            {
               ...
               if(npc.getHitTime() >= CultimaPanel.numFrames && !player.getWeapon().getName().toLowerCase().contains("lightning"))
               {
                  Color weapColor = colorForWeapon(player.getWeapon());
                  g.setColor(weapColor);
                  for(int thickness=-2; thickness<=2; thickness++)
               ...
               //make a flash at the point of impact
                  ...
                  {
                     float mindPercent = (float)(player.getMind()) / Player.MAX_STAT;
               ...
               if(npc.getMissTime() >= CultimaPanel.numFrames && !player.getWeapon().getName().toLowerCase().contains("lightning"))
               {
               ...
               if(npc.getAttackHitTime() >= CultimaPanel.numFrames || npc.getAttackMissTime() >= CultimaPanel.numFrames)
               {
                  ...
                  if(targetLoc.getRow()!=0 && targetLoc.getCol()!=0)
                  {
                     ...
                     if(targX!=-1 && targY!=-1)
                     {
                        if(npc.getCharIndex()==NPC.SORCERER && npc.getWeapon().getName().toLowerCase().contains("lightning"))
                        {} //the sorcerer's lightning is drawn from the sky, so don't show an attack vector from the sorcerer
                        else
                        {
                           for(int thickness=-2; thickness<=2; thickness++)
                           {
               ...
               if(!ter.getName().equals("TER_S_B_$Dense_forest") || npc.canFly() || NPC.getSize(npc) > 2)
               {
                  ...
                  if(missionPriority > 0 && npc.getCharIndex()!=NPC.GORGON)
                  ...
                  if(npc.getCharIndex()==NPC.MINDDEVOURER || ((npc.getCharIndex()==NPC.BEHOLDER || npc.getCharIndex()==NPC.LICHE) && npc.getNumInfo() > 0))
                  {
                     Color mannaColor = (magicColor.brighter().brighter().brighter().brighter());
                     Color healthColor = Color.red;
                     int opacity = 150;
                     int frame10 = CultimaPanel.numFrames % 100;
                     if(frame10 < 50)
                     {
                        opacity = 150 - frame10;
                     }
                     ...
                     {
                        opacity = 100 + (50-frame10);
                     }
                     int glowSize = SIZE+frame10;
                     int glowMod = (int)(glowSize/3);
                     if(npc.getCharIndex()==NPC.MINDDEVOURER || npc.getNumInfo() == 1)
                     {
                        g.setColor(new Color(mannaColor.getRed(),mannaColor.getGreen(),mannaColor.getBlue(),opacity));
                        g.fillOval(npcX-glowMod, npcY-glowMod, glowSize, glowSize);
                     }
                     else if(npc.getNumInfo() == 2)
                     {
                        g.setColor(new Color(healthColor.getRed(),healthColor.getGreen(),healthColor.getBlue(),opacity));
                        g.fillOval(npcX-glowMod, npcY-glowMod, glowSize, glowSize);
                     }
                  }
                  else if(npc.getLocationType().contains("pacrealm") && npc.getCharIndex()==NPC.GHOST)
                  {
                     Color dotColor = (Color.black);
                     int alpha = 50;
                     if((System.currentTimeMillis() <= CultimaPanel.alertTime + 4000) || (System.currentTimeMillis() <= CultimaPanel.alertTime + 6000 && CultimaPanel.numFrames%10 < 5))
                     {  
                        dotColor = (Color.blue.brighter().brighter());
                        alpha = 100;
                     }
                     ...
                     g.fillOval(npcX-(SIZE/2), npcY-(SIZE/2), SIZE*2, SIZE*2);
               ...
               if(timestop==null)
               {
                  ...
                  {
                     if(npc.getCharIndex()==NPC.GORGON && npc.getNumInfo() > 0)
                     {
                        int radius = (int)(SIZE * npc.getNumInfo());
                        Color glow = Color.cyan;
                        if(Math.random() < 0.5)
                        {
                           glow = Color.cyan.brighter();
                        }
                        int opacity = Math.min(255, Math.max(0,Utilities.rollDie(25, 75)));
                        g.setColor(new Color(glow.getRed(),glow.getGreen(),glow.getBlue(),opacity));
                        g.fillOval(npcX-(radius/4), npcY-(radius/4), radius, radius);
                     }
                     else if((npc.getWeapon().getEffect().toLowerCase().contains("fire") && !npc.getWeapon().getName().toLowerCase().contains("cannon") ) || npc.hasEffect("fire") || (npc.getCharIndex()==NPC.WILLOWISP && !npc.getLocationType().contains("pacrealm")) || npc.getCharIndex()==NPC.PANGOLIN || npc.getCharIndex()==NPC.ROLLINGMENACE)
                     {
               ...
               String npcName = npc.getName().toLowerCase();
               if(npc.getCharIndex()!=NPC.GORGON && (missionPriority > 0 || (npc.getBounty()>0 || (missionTargetNames!=null && missionTargetNames.contains(Utilities.shortName(npc.getName()))))))
               {  //mark npcs that have missions and npcs that hold objectives (red) - skip for gorgons so we can see their charge-up glow for their special attack
                  Color dotColor = Color.red;
   ...
   public static ArrayList<Location> closestLocationsTo(int row, int col)
   {
      ...
      for(int i=0; i<found.size(); i++)
      {
         ...
         {
            float distMin = wrapDistance(row, col, found.get(min).getRow(), found.get(min).getCol());
            float distJ = wrapDistance(row, col, found.get(j).getRow(), found.get(j).getCol());
            if(distJ < distMin)
               min = j;
         }
         swap(found, i, min);
   ...
   public static Location closestLocationTo(int row, int col)
   {
      ...
      for(int i=1; i< CultimaPanel.allLocations.size(); i++)
      {
         ...
         if(loc.getFromMapIndex() == 0)
         {
            float dist = wrapDistance(row, col, loc.getRow(), loc.getCol());
            if(dist < minDist)
            {
               minDist = dist;
               closest = loc;
...
//given the name of a location, returns it without a prefix or suffix
   ...
   {  //we can have thunderstroms from mid spring through mid fall
      if(CultimaPanel.player.getLocationType().contains("pacrealm") || CultimaPanel.player.getLocationType().contains("frogrealm"))
         return false;
      int rem = (days + player.getStartDay()) % 60;         
  ...
  //true if it is summer
   ...
   public static float distFromDeadWinter()
   {
      if(!isWinter())
         return 0;
      int rem = (days + player.getStartDay()) % 60;         
      return 1.0f - (Math.abs(38 - rem)/7.5f);
   }
   ...
   public static Color findGroundColor(boolean first)
   {
      ...
      if(player.getLocationType().contains("frogrealm"))
      {
         return spring;
      }
  ...
  //post: returns the color to use for darkened or hidden spaces, accounting for if it is cloudy, indoor or outdoor
   public static Color timeColor(float time, String locationType)
   {  //                            "Dead Of Night",        "Pre Dawn",          "Dawn",                 "Early Morn",           "Morning",                   "Late Morn",            2 PM "Afternoon",         4 PM "Late Afternoon",    6 PM "Pre Dusk",   8 PM "Dusk",           10 PM "Evening",        12 PM "Midnight"   
      ...
      float percentRed = ((time - lowerTime)/2)*100; //percentage of difference between time and lowerTime
      float percentGreen = ((time - lowerTime)/2)*100; //percentage of difference between time and lowerTime
      float percentBlue = ((time - lowerTime)/2)*100; //percentage of difference between time and lowerTime
   ...
   public static float distanceSimple (int x1, int y1, int x2, int y2)
   {
      return (float)(Math.sqrt(((x2-x1)*(x2-x1)) + ((y2-y1)*(y2-y1))));
   }

   public static float findDistance(int x1, int y1, int x2, int y2)
   {
      if(player.getMapIndex()==0)
         return wrapDistance(x1, y1, x2, y2);
      return (float)(distanceSimple(x1, y1, x2, y2));
   }

//distance between cells with wrap-around depending on map dimensions, assuming we are in the greater world
   public static float wrapDistance(int x1, int y1, int x2, int y2)
   {
      int mapIndex = 0;
      ...
      if(dy < numRows/2)
         newDy = dy;
      return (float)(Math.sqrt(newDx*newDx + newDy*newDy));
   }
...
//returns List of (r,c) Terrain locations that should be hidden by darkness (determined by time of day)
   public static ArrayList<Coord> darkenedCells(float tempTime)
   {
      int mapIndex = player.getMapIndex();
      ...
      if((flashColor != null && numFrames <= flashFrame+Math.max(3,animDelay/2)) || player.getLocationType().equals("pacrealm") || player.getLocationType().equals("frogrealm"))
         return temp;
   ...
   //see if player has a light spell
      ...
      final float torchFlickerChance = 0.23f;
      final float flameFlickerChance = 0.15f;
      String weapName = player.getWeapon().getName().toLowerCase();
      String armName = player.getArmor().getName().toLowerCase();
      ...
      for(Coord p: lights)
      {
         ...
         for(int i=0; i<temp.size(); i++)
         {
            ...
            int darkR = d.getRow();
            int darkC = d.getCol();
            float dist = wrapDistance(lightR, lightC, darkR, darkC);
            if(dist <= value)
            {
               temp.remove(d);
               i--;
   ...
   public static ArrayList<Coord> blockedView()
   {
      ...
      Spell flight = player.getActiveSpell("Flight");
      boolean indoors = TerrainBuilder.indoors();
      if(currentPos.getName().contains("Mountain") || currentPos.getName().contains("Mountain") || player.getLocationType().equals("ship") || (flight!=null && !indoors) || player.getLocationType().equals("pacrealm") || player.getLocationType().equals("frogrealm"))  
   ...
   public static final String [][] readMePages = {
     ...
     /*page 9*/
     {
        ...
        "     Pangolin art and conept by Mia Monroe",
        "",
