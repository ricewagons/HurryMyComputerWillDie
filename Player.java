...
//fear spell illusions
   ...
   protected static final float EXP_COST_PER_LEVEL = 1.10f;    //10% increase in # experience points to get to next level
   ...
   private float hitTime,missTime;    //used to show hit graphic when the player gets damaged
   ...
   private float campStartTime;       //to keep track of number of hours of rest
   ...
   private byte currTerrain;           //the value of the current terrain we are on, used to set the color of a moving platform when we break it away from a ledge in the underworld 
...
//***INFO-INDEXES
   ...
   public static final byte LOCATION_INFO = 16;       //index for a list of location information
   public static final byte NUM_INFO_INDEXES = 17;    //the number of information indexes (and infoIndexes array size)
...
//***STAT KEEPING
  ...
  //**************
   ...
   public Player(String name, ArrayList<String> images, boolean allowAffliction)
   {
      ...
      infoIndexes[LOCATION_INFO] = 80;    //we start with an understanding of basic locations, but have to learn about the underworld
      ...
      if(Math.random() < 0.1)
      {
         ...
         infoIndexes[LOCATION_INFO] = NPC.locationInfo.length;
      }
   ...
   int[] specExp, int [] statArray, int [] infoInd, Coord [] weapHKeys, int [] spellHKeys, int []potionHKeys)
   {
      ...
      if(infoIndexes[CRAFTING_INFO] > NPC.gemInfo.length)
         infoIndexes[CRAFTING_INFO] = NPC.gemInfo.length;
      if(infoIndexes[LOCATION_INFO] > NPC.locationInfo.length)
         infoIndexes[LOCATION_INFO] = NPC.locationInfo.length;
   ...
   public void setSulliedValue(int sv)
   {
      ...
      float percentSullied = ((float)(sulliedValue)/this.getBody())*100;
   ...
   public byte getCurrentTerrain()
   {
      return currTerrain;
   }
   
   public void setCurrentTerrain(byte ct)
   {
      currTerrain = ct;
   }
   ...
   {
      return (!onHorse() && !isSailing() && !isFlying() && !isWolfForm()) && (currentPos.getName().toLowerCase().contains("tall_grass") || currentPos.getName().toLowerCase().contains("water") || (currentPos.getName().toLowerCase().contains("bog") && !Display.isWinter()) || (currentPos.getName().toLowerCase().contains("forest") && !currentPos.getName().toLowerCase().contains("dead")));
   }
   
   public boolean canHide()
   {
      if(onHorse() || isSailing() || isFlying() || isWolfForm())
      {
         return false;
      }
      if(mapIndex >=0 && mapIndex < CultimaPanel.map.size())
      {
         ...
         if(!LocationBuilder.outOfBounds(currMap, row, col))
         {
            ...
            return (currentPos.getName().toLowerCase().contains("tall_grass") || currentPos.getName().toLowerCase().contains("water") || (currentPos.getName().toLowerCase().contains("bog") && !Display.isWinter()) || (currentPos.getName().toLowerCase().contains("forest") && !currentPos.getName().toLowerCase().contains("dead")));
         }
      }
      return false;
   }
   ...
   {
      return infoIndexes[LOCATION_INFO];
   }
   ...
   //has a weapon that can bash the ground to separate a section of ground next to liquid in the underworld
   public boolean hasBashWeapon()
   {
      return (imageIndex!=NONE && imageIndex!=LUTE && imageIndex!=THROWN && imageIndex!=BOW && imageIndex!=CROSSBOW && imageIndex!=TORCH);
   }
   
   //can the player bash the ground to split it off to float across a liquid like lava or water
   public boolean canBashGround()
   {
      return (this.getLocationType().contains("underworld") && (Utilities.nextToLiquidLeft() || Utilities.nextToLiquidRight()) && this.hasBashWeapon());
   }
   ...
   public boolean isInSpecialRealm()
   {
      return (this.getLocationType().toLowerCase().contains("underworld") || 
             ...
             this.getLocationType().toLowerCase().contains("frogrealm") ||
   ...
   public void setCamping(boolean state)
   {
      ...
      for(int r=row-5; r<=row+5; r++)
      {
         for(int c=col-5; c<=col+5; c++)
         {
            if(!LocationBuilder.outOfBounds(currMap, r, c))
            {
               ...
               else if(carpet.contains("gray_tile_wall"))
               {
                  inHauntedHouse = true;
                  break;
               }
      ...
      if(inHauntedHouse)
      {
         for(int i=0; i<CultimaPanel.missionStack.size(); i++)
         {
            ...
            if(m.getMissionType()==Mission.HAUNTED_DARE && !this.hasItem("haunted-night-journal-entry"))
            {
               this.addItem("haunted-night-journal-entry");
            }
         }
      }  
      if(state)
      ...
      else           //state is false
      {
         ...
         float healPerc = 0.05f;
         if(isVampire() && current.contains("coffin"))
            healPerc = 0.2f; 
         else
         {                                                              //the quality of our rest depends on how comfortable our resting place is
            if(this.inHauntedHouse())
               healPerc = 0.025f;
            else if(current.contains("bed"))
               healPerc = 0.075f; 
            else if(current.contains("flower"))
               healPerc = 0.065f; 
            else if(current.contains("grass"))
               healPerc = 0.055f;
            else if(current.contains("forest"))
               healPerc = 0.045f;
            else if(current.contains("sand"))
               healPerc = 0.035f;
            else if(current.contains("hill"))
               healPerc = 0.025f;
         }
         if(inHome)                                                     //better rest if in the comfort of our own home
         {
            healPerc += 0.025;
         }
         if(withSpouse)                                                 //better rest if with our spouse
         {
            healPerc += 0.025;
         }
         if(!dogBasicInfo.equals("none"))                               //we get better rest if we have a dog guarding us while camping
         {
            healPerc += 0.025;  
         }  
      ...
      if(effects!=null && effects.contains(eff))
      {
         for(int i=0; i<effects.size(); i++)
            if(effects.get(i).equals(eff))
            {
               ...
               {
                  for(int j=0; j<statMod.length; j++)
                  {
                     statMod[j] = 0;
                  }
               }
...
//remove all effects
   ...
   public void scrollWeaponRight()
   {
      ...
      int numSellable = 0;
      if(CultimaPanel.menuMode == CultimaPanel.SHOPPE_ARMORY || CultimaPanel.menuMode == CultimaPanel.SHOPPE_TRADER || this.getLocationType().contains("pacrealm") || this.getLocationType().contains("frogrealm"))
      ...
      else if( (CultimaPanel.menuMode == CultimaPanel.SHOPPE_ARMORY || CultimaPanel.menuMode == CultimaPanel.SHOPPE_TRADER || this.getLocationType().contains("pacrealm") || this.getLocationType().contains("frogrealm")) && numSellable >= 1 && (this.getImageIndex()==STAFF || this.getImageIndex()==NONE || this.getImageIndex()==THROWN))
         scrollWeaponRight();       //scroll past things we can't sell
      else if(weapName.contains("lightning") && indoors)
         scrollWeaponRight();       //scroll past a weapon we can't use because we must be outdoors 
   }
   ...
   public void scrollWeaponLeft()
   {
      ...
      if((this.getImageIndex()==STAFF) && (!this.hasStaffReadied()) && (this.getMannaMax() < (weapons[imageIndex].get(weaponSelect).getMannaCost())))
         ...
         scrollWeaponLeft();       //scroll past a weapon we can't use because we must be outdoors 
   }

//returns whether or not we know that spell
   public boolean knowsSpell(String s)
   {
      for(Spell sp:spells)
      {
...
//return the active spell at index select if we could cast it
   ...
   {
      boolean indoors = TerrainBuilder.indoors();
      if(getImageIndex()==WOLF || this.getLocationType().contains("pacrealm") || this.getLocationType().contains("frogrealm"))
         return null;  
      ...
      for(int i=0; i<3; i++)  //traverse through 2 pssible groups of NPCs: monsters and civilians
      {
         ...
         for(NPCPlayer p: group)
         {
            if(p.getMapIndex()!=this.getMapIndex())
               continue;
            float dist = Display.findDistance(p.getRow(), p.getCol(), this.getRow(), this.getCol());
            if(dist > range)
               continue;   
            for(int eff=0; eff<effect.length; eff++)
            {
               float effectChance = 0.9f;    //repel has a 90% chance of stunning the targets
            ...
            float damageChance = 0.9f;    //repel has a 90% chance of damaging
...
//cast the spell at index selected (without changing to that selected spell - used for hot-keys)
   public Spell castSpell(byte selected)
   {
      ...
      {  
         ...
         boolean indoors = TerrainBuilder.indoors();      //used to see if fire is less potent in the rain, or freeze more potent in the rain
         ...
         else                       //instant spell
         {
            ...
            else if(curr.getName().toLowerCase().contains("tame-animal"))
            {
               ...
               for(NPCPlayer p: CultimaPanel.worldMonsters)
               {
                  ...
                  float dist = Display.findDistance(this.getRow(), this.getCol(), p.getRow(), p.getCol());
            ...
            else if(curr.getName().contains("Raise-earth"))
            {
               ...
               for(int r=this.getRow() - 2; r<=this.getRow()+2; r++)
               {
                  ...
                  {
                     float dist = Display.distanceSimple(r, c, this.getRow(), this.getCol());
...
//decrement the counter for each active spell by 1 and remove those that are done
   ...
   public void runEffects()
   {
      ...
      float winter = Display.distFromDeadWinter();
      ...
      {
         int damMod = 2;
         float catchOnFire = onUs.getPercentHealth()*1.5f;
      ...
      if(!indoors && !sailing && winter > 0.33 && !this.wearingFur() && this.getImageIndex()!=TORCH && !nextToHeatSource && Math.random() < (winter/2.0) && CultimaPanel.weather >= 3 && !this.getLocationType().toLowerCase().contains("frogrealm"))
      {  //damage from snow in winter without a pelt
         int damage = 1;
         CultimaPanel.sendMessage(Utilities.getRandomFrom(cold));
         damage(damage);
      }
      if(!sailing && !flight && winter > 0 && playerTerrain.contains("water") && !Display.frozenWater() && Math.random() < 0.25 && !this.getArmor().getName().toLowerCase().contains("seaserpent") && !this.getLocationType().toLowerCase().contains("underworld") && !this.getLocationType().toLowerCase().contains("frogrealm"))
      {  //damage from swimming in winter
         int damage = (int)(1 + winter*5);
         CultimaPanel.sendMessage(Utilities.getRandomFrom(coldWater));
         damage(damage);
      }
      if(CultimaPanel.healthDrain && Math.random() < 0.1)
      {
         damage(1);
      }
...
//returns true if player has enough room to add the arg weapon toAdd to their inventory
   public boolean canAddWeapon(Weapon toAdd)
   {
      ...
      if(toAdd.getImageIndex()==DAGGER || toAdd.getName().toLowerCase().contains("trap") || toAdd.getName().toLowerCase().contains("exotic"))   
         toAddWeight = 0.5f;      //daggers, traps and weapons made with exotic metal are half the weight
      ...
      float totalWeight = getWeaponWeight();
...
//awareness without randomness for stable NPC stat viewing with focus spell
   ...
   public void setMindNoSound(int m)
   {
      mind = m;
      painTime = CultimaPanel.numFrames + (CultimaPanel.animDelay*3);
   }
   ...
   public void damage(int d)
   {
      ...
      if(this.getBody() < 0)
      {
         ...
         if(this.getLocationType().contains("frogrealm"))
         {
            Sound.dieInFrogRealm();
         }
      ...
      if(this.getBody() <= 0)
      {
         ...
         if(hasItem("life-pearl") && !this.getLocationType().contains("frogrealm") && !this.getLocationType().contains("pacrealm"))
            CultimaPanel.sendMessage("Life escapes thee, but thy life-pearl glows!");
         else
            if(hasItem("blessed-crown"))
               CultimaPanel.sendMessage("You ascend to the Gods as a cousin of Skara Brae!");
            else
   ...
   {
      if(this.getBody() > 0 && !CultimaPanel.mannaDrain && !CultimaPanel.nearHauntedHouse)
      {
         manna += m;
         if(manna > this.getMannaMax())
            manna = this.getMannaMax();
...
//for changing to Flight or a Ship
   ...
   public void setHitTime(float ht)
   {
      hitTime = ht;
   }

   public float getMissTime()
   {
      return missTime;
   }

   public void setMissTime(float ht)
   {
      missTime = ht;
   }
   ...
   { 
      ...
      {         
         ...
         {
            float totalCoords = 0;          //to make specific experience guide the quality we increase when we level up
            for(int pt: specificExperience)  //find the percentage of each specific experience points out of the total
               totalCoords += pt;
            float[] expPercentages = new float[specificExperience.length];
  ...
  //returns the target if we want to force the game to talk with them (assigned to CultimaPanel.selected), or null if not
   public NPCPlayer attack(NPCPlayer target, boolean altAttack)
   {
      ...
      if(weapName.contains("phasewall") && target != null && !target.isStatue() && target.getCharIndex()!=NPC.MONOLITH)
         return null;
      if(weapName.contains("lightning") && indoors)
      {
         CultimaPanel.sendMessage("Thou must be under the sky to summon lightning!");
         return null;   
      }
      ...
      String spellName = Weapon.spellChant(weapName);
      if(spellName.length() > 0)
      {
         CultimaPanel.sendMessage("-----");
         CultimaPanel.sendMessage(spellName);
      }
      ...
      float mindPercent = (float)(mind) / MAX_STAT;       //the percentage of mind attribute as a decimal - used for random throws
      float agilityPercent = (float)(agility) / MAX_STAT; //the percentage of agility attribute as a decimal - used for random throws
      if(isMagic)
      {
         ...
         if(heightBonus > 1 || this.hasItem("Magestaff") || this.hasItem("Ohnyalei"))
            bonusTemp = 1.5f;
         if(weapon.getName().toLowerCase().contains("blind"))
            bonusTemp *= 3;    
         else if(target.hasEffect("control") || target.hasEffect("freeze") || target.hasEffect("web") || target.hasEffect("stun"))
            bonusTemp *= 2;   
         ...
         {                    //we hit our target
            ...
            if(weapName.contains("lightning"))
            {  //record start and end lightningNodes from player to target
               int startX = (CultimaPanel.SCREEN_SIZE/2)*CultimaPanel.SIZE +Utilities.rollDie(-25,25);
               int startY = 0;
               Coord lightningStart = new Coord(startX, startY);
               Coord lightningEnd = new Coord(target.getNpcX(), target.getNpcY());
               Utilities.connectLightning(lightningStart, lightningEnd);
               int radius = Utilities.rollDie(5,10);
               int opacity = Utilities.rollDie(75,127);
               int plX = lightningEnd.getRow();
               int plY = lightningEnd.getCol();  
               for(int i=0; i<10 && CultimaPanel.smoke.size()<50; i++)
               {
                  CultimaPanel.smoke.add(new SmokePuff(plX+Utilities.rollDie(-6,6), plY+Utilities.rollDie(-6,6), radius, opacity, Color.black));
               }
            }
         }
         else //we missed
         {
            if(weapName.contains("lightning"))
            {  //record start and end lightningNodes from player to someplace around target
               Coord lightningStart = new Coord((CultimaPanel.SCREEN_SIZE/2)*CultimaPanel.SIZE+Utilities.rollDie(-25,25), 0);
               Coord lightningEnd = new Coord(target.getNpcX()+Utilities.rollDie(-25,25), target.getNpcY()+Utilities.rollDie(-25,25));
               Utilities.connectLightning(lightningStart, lightningEnd);
               int radius = Utilities.rollDie(5,10);
               int opacity = Utilities.rollDie(75,127);
               int plX = lightningEnd.getRow();
      ...
      {//higher chance of hitting our target if we are closer
         ...
         if(heightBonus > 1 || Weapon.isLegendaryWeapon(weapName))
            bonusTemp = 1.25f;
         if(target.hasEffect("control") || target.hasEffect("freeze") || target.hasEffect("stun")  || target.hasEffect("web") || ((NPC.isUndead(target.getCharIndex()) || target.isZombie()) && weapName.contains("life")) || NPC.isShip(target.getCharIndex()))
            bonusTemp *= 2;
      ...
      else //if (isMelee)
      {
         ...
         if(heightBonus > 1 || Weapon.isLegendaryWeapon(weapName) || isShieldBash)
            bonusTemp = 1.25f;
         if(target.hasEffect("control") || target.hasEffect("freeze") || target.hasEffect("stun") || target.hasEffect("web") || isShieldBash || ((NPC.isUndead(target.getCharIndex()) || target.isZombie()) && weapName.contains("life"))  || NPC.isShip(target.getCharIndex()))
            bonusTemp *= 2;
         ...
         {                    //we hit our target
            ...
            if(Weapon.isLegendaryWeapon(weapName) && mightyBlow)
               legendBonus = 1.5f;        //higher chance of critical hit if we have a legendary weapon or we are doing an alternate mighty blow with a sword or hammer
            else if(Weapon.isLegendaryWeapon(weapName) || mightyBlow)
               legendBonus = 1.25f;        //higher chance of critical hit if we have a legendary weapon or we are doing an alternate mighty blow with a sword or hammer
      ...
      if((target.getCharIndex()==NPC.DEMON || target.getCharIndex()==NPC.DEMONKING) && distance >= 3 && Math.random() < 0.25)
      {
         ...
         if(teleSpots.size() > 0)
         {
            ...
            CultimaPanel.sendMessage("-----");
            CultimaPanel.sendMessage("~Tace Moventur!");
         }   
      ...
      if(target.getCharIndex() == NPC.LANDSHARK && !target.hasEffect("stun"))
      {
         dam = 0;
      }
      if(target.getCharIndex()==NPC.GHOST && target.getLocationType().contains("pacrealm"))
      {
         dam = 1000;
      }
      if(dam > 0 || turnedStone)
      {
         ...
         if((isGarriott || target.getCharIndex()==NPC.SLIME || target.getName().equals("Skara Brae") || target.getCharIndex()==NPC.PANGOLIN  || target.getCharIndex()==NPC.ROLLINGMENACE) && Math.random() < 0.75)       //Richard Garriott has awesome armor
            dam /= 2;
         ...
         { 
            if(!weapon.getEffect().contains("fire"))
            {
               ...
               float save = (float)Math.max(0, (1 - (roll/100.0)));
               dam = (int)(dam * save);
            }
         ...
         float arrowRecoverProb = 0.75f;
         float magicArrowRecoverProb = 0.5f;
         if(this.hasItem("swiftquill") || this.hasItem("bow-bracer"))
         {
            arrowRecoverProb = 0.95f;
            magicArrowRecoverProb = 0.75f;
         }
         if(this.hasItem("swiftquill") && this.hasItem("bow-bracer"))
         {
            arrowRecoverProb = 0.975f;
            magicArrowRecoverProb = 0.8f;
         }
         ...
         //remove target, add to restore, add Corpse, adjust reputation
            ...
            {   
               ...
               else
               {
                  ...
                  Corpse corpseThere = Utilities.getCorpseAt(target.getRow(), target.getCol(), target.getMapIndex());
                  if(corpseThere != null)
                  {
                     int corpseGold = corpseThere.getGold();
                     int corpseArrows = corpseThere.getNumArrows();
                     ArrayList<Item> corpseItems = corpseThere.getItems();
                     Weapon corpseWeapon = corpseThere.getWeapon();
                     Armor corpseArmor = corpseThere.getArmor();
                     Weapon deadWeapon = dead.getWeapon();
                     Armor deadArmor = dead.getArmor();
                     corpseThere.clearItems();
                     dead.setGold(dead.getGold() + corpseGold);
                     dead.setNumArrows(dead.getNumArrows() + corpseArrows);
                     if(corpseItems != null && corpseItems.size() > 0)
                     {
                        for(Item corpseItem: corpseItems)
                        {
                           dead.addItem(corpseItem);
                        }
                     }
                     if(corpseWeapon != null && (deadWeapon==null || corpseWeapon.getValue() > deadWeapon.getValue()))
                     {
                        dead.setWeapon(corpseWeapon);
                        corpseThere.setWeapon(deadWeapon);
                     }
                  }
   ...
   public boolean inHauntedHouse()
   {
      byte[][]currMap = (CultimaPanel.map.get(this.getMapIndex()));
      for(int rr=this.getRow()-2; rr<=this.getRow()+2; rr++)
      {
         for(int cc=this.getCol()-2; cc<=this.getCol()+2; cc++)
         {
            if(!LocationBuilder.outOfBounds(currMap, rr, cc))
            {
               String wall = CultimaPanel.allTerrain.get(Math.abs(currMap[rr][cc])).getName().toLowerCase();  
               if(wall.contains("gray_tile_wall"))
               {
                  return true;
               }
            }
      ...
      return false;
         ...
         //returns a rest messing when we set camp or sleep depending on the terrain we are on
   public String restMessage(String terrainName)
   {
      String [] hauntedHouse = {"This unsettling house will make for uneasy sleep...", "I feel uneasy about resting near this rotting house...", "I feel as if angry spirits from this house hang over me...", "A chill and a foul smell spill from this house..."};
      String [] bed = {"I shall get excellent rest in this comfortable bed...", "This cofortable bed will provide excellent rest...", "This bed will provide thee excellent rest...", "Excellent rest is certain in this comfortable bed..."};
      String [] flowers = {"I shall get good rest amongst the petals...", "This patch of petals will provide good rest...", "This bed of flowers will provide thee good rest...", "Good rest is certain amongst the flowers..."};
      String [] grass = {"I shall get fine rest in this patch of grass...", "This grassy patch will provide fine rest...", "This patch of grass will provide thee fine rest...", "Fine rest is certain in this grass..."};
      String [] forest = {"Camping amongst these trees will have to do...", "Beneath these trees will suffice for rest...", "Amongst these trees is sufficient for camp...", "Sufficient rest will come from camping amongst these trees..."};
      String [] sand = {"Camping in this gritty sand is uncomfortable...", "This coarse sand makes for an uncomfortable resting place...", "Coarse sand is disagreeable for resting upon...", "Uncomfortable rest will come from camping on this sand..."};
      String [] hill = {"T'will be difficult to rest on these rocky hills...", "These rocky hills make for a difficult nights rest...", "Rocky hills are difficult for resting upon...", "Difficult rest will come from camping on this rocky hill..."};
      if(this.inHauntedHouse())
         return Utilities.getRandomFrom(hauntedHouse);
      if(terrainName.contains("bed"))
         return Utilities.getRandomFrom(bed);
      if(terrainName.contains("flowers"))
         return Utilities.getRandomFrom(flowers);
      if(terrainName.contains("grass"))
         return Utilities.getRandomFrom(grass);   
      if(terrainName.contains("forest"))
         return Utilities.getRandomFrom(forest);   
      if(terrainName.contains("sand"))
         return Utilities.getRandomFrom(sand);
      if(terrainName.contains("hill"))
         return Utilities.getRandomFrom(hill);
      return "'Tis a good time to rest...";
   }
