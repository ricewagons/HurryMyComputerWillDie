...
public class Weapon extends Item
{
  ...
  //               name       effect  minDamage maxDamage  range  statModifier value imageIndex reloadTime mannaCost
   ...
   {
      int modifier = 0;
      if((this.getName().toLowerCase().contains("bow") || this.getName().toLowerCase().contains("caster")) && this.getRange() >= 3)
      {
         modifier = (50 - CultimaPanel.player.getAgility())/3;
      }
      else if(this.getRange() >= 2 || isDagger(this.getName()))
      {  //modifier depends on agility
         modifier = (50 - CultimaPanel.player.getAgility());
      }
      else
      {  //modifier depends on strength
         modifier = (50 - CultimaPanel.player.getMight());
      }
      return reloadFrames + modifier;
   }
   ...
   public static Weapon getRandomSpecialBow()
   {
      ArrayList<Weapon> weapons = new ArrayList<Weapon>();
      weapons.add(getWindBow());    
      weapons.add(getBrightLongbow());    
      weapons.add(getFlamebow());    
      weapons.add(getFrostbow());
      weapons.add(getBowOfKarna());
      return weapons.get((int)(Math.random()*weapons.size()));
   }
...
//npc sorcerer's lightning attack
   ...
   public static Weapon getBurnination()
   {
      return new Weapon("Burnination", "fire", 50, 100, 6, null, 0, Player.NONE, 0, 0);
   }
   ...
   //given the spell weapon name, returns the chant the player says to cast it
   public static String spellChant(String n)
   {
      n = n.toLowerCase().trim();
      if(n.contains("phasewall"))
         return "Murus Evanescet!";
      if(n.contains("advance"))
         return "Tace Moventur!";
      if(n.contains("curse"))
         return "Maledicere Inimicus!";
      if(n.contains("possess"))
         return "Hostes Possident!";
      if(n.contains("fireball"))
         return "Ignem Mittent!";
      if(n.contains("spidersweb"))
         return "Araneae Laqueus!";
      if(n.contains("icestrike"))
         return "Glaciem Percutiens!";
      if(n.contains("lightning"))
         return "Vocare Fulgur!";
      if(n.contains("deathtouch"))
         return "Mors Tactus!";
      if(n.contains("fireshield"))
         return "Ignis Scutum!";
      if(n.contains("summon-vortex"))
         return "Vocare Vorticem!";
      if(n.contains("raise-stone"))
         return "Attollere Lapis!";
      if(n.contains("lightningbolt"))
         return "Fulmina Arcesse!";
      if(n.contains("stonecast"))
         return "Lapis Rursus!";
      if(n.contains("blindinglight"))
         return "Lux Caeca!";
      return "";
   }
   ...
   public static Weapon getWeaponWithName(String n)
   {
      ...
      if(n.equalsIgnoreCase("burnination"))
         return getBurnination();
