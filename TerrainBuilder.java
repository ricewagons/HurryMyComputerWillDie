                     ...
                     if(current != null)
                     {
                        ...
                        { 
                           ...
                           {  //if it is a place populated with people, make it less likely to find forest and tall grass in the center
                              float distToCenter = Display.findDistance(r, c, center, center);
                              float percFromCenter = 1 - (float)(distToCenter/(Math.sqrt((center*center) + (center*center))));
...
//returns true if world[r][c] is campable for player
  ...
  //returns true if player is likely in a building - used for rain and torch lighting like we are indoors
   public static boolean indoors()
   {
      ...
      String locType = CultimaPanel.player.getLocationType().toLowerCase();
      if(locType.contains("sewer") || locType.contains("dungeon") || locType.contains("cave") || locType.contains("mine") || locType.contains("lair") || locType.contains("wolfenstein") || locType.contains("pacrealm"))
         return true;    
      int r = CultimaPanel.player.getRow();
      int c = CultimaPanel.player.getCol();
      int mi = CultimaPanel.player.getMapIndex();  
      ...
      String currName = current.getName().toLowerCase();
      if((locType.contains("castle") || locType.contains("tower")) && !currName.contains("wood_plank") && !currName.startsWith("ter_"))
         return true;
...
//fills up world with locations and returns the list of all Locations added
     ...
     //add special endgame temple at 50x50
      ...
      for(int i=0; i<numSeeds; i++)
      {
         ...
         while(!(temp.getNeighbors()).containsKey(CultimaPanel.allTerrain.get(Math.abs(world[r][c])).getName()) || onWater(world, r, c) 
            ...
            getLocationAt(allLocations, r, CultimaPanel.equalizeWorldCol(c+1), 0)!=null || 
            getLocationAt(allLocations, r, CultimaPanel.equalizeWorldCol(c-1), 0)!=null)
         {
            r = (int)(Math.random()*world.length);
            c = (int)(Math.random()*world[0].length);
      ...
      //only make possibility for a port town if it is vertcally or horizontally adjacent to water with at least one diagonal water border as well
         ...
         else if((type.equals("town") || type.equals("port") || type.equals("capital")) || (type.equals("temple") && Math.random() < 0.75) || ((type.equals("mine") || type.equals("lair")) && Math.random() < 0.5) || ((type.equals("cave")) && Math.random() < 0.25))
         { 
            if(r==50 && c==50)
            {}    //no path for the temple of skara-brae
            else
            {  
               ArrayList <Byte> pathDirs = new ArrayList<Byte>();  //NORTH, WEST, SOUTH, EAST
               String terrName = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r-1)][CultimaPanel.equalizeCol(c)])).getName().toLowerCase();
               if(!terrName.contains("grassland") && !terrName.contains("dry_grass") && !terrName.contains("sand") && !terrName.startsWith("loc_"))
               {
                  pathDirs.add(NORTH);
               }
               terrName = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c+1)])).getName().toLowerCase();
               if(!terrName.contains("grassland") && !terrName.contains("dry_grass") && !terrName.contains("sand") && !terrName.startsWith("loc_"))
               {
                  pathDirs.add(EAST);
               }
               terrName = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r+1)][CultimaPanel.equalizeCol(c)])).getName().toLowerCase();
               if(!terrName.contains("grassland") && !terrName.contains("dry_grass") && !terrName.contains("sand") && !terrName.startsWith("loc_"))
               {
                  pathDirs.add(SOUTH);
               }
               ...
               if(pathDirs.size() >= 1)
               {
                  int numPaths = Utilities.rollDie(1, pathDirs.size());
                  if(type.equals("temple") || type.equals("mine") || type.equals("lair") || type.equals("cave"))
                  {
                     numPaths = 1;
                  }
                  for(int np=0; np<numPaths && pathDirs.size() > 0; np++)
                  {
                     byte pathDir = pathDirs.remove((int)(Math.random()*pathDirs.size()));
                     int startRow = r, startCol = c;
                     if(pathDir == NORTH)
                     {
                        startRow = r-1;
                     }
                     else if(pathDir == EAST)
                     {
                        startCol = c+1;
                     }
                     else if(pathDir == SOUTH)
                     {
                        startRow = r+1;
                     }
                     else //if(pathDir == WEST)
                     {
                        startCol = c-1;
                     }
                     placePath(type, world, pathDir, startRow, startCol);
                     placeBridge(world, r, c);
                  }
               }
            }
         }
  ...
  //for each side surrounded by mountain, hills, forest, swamp - possibly put down a dry grass path to the first cell that isn't. For each side surrounded by water or lava, possibly put down a bridge if it could reach land in 8 spaces
   public static void placePath(String type, byte[][]world, byte pathDir, int startRow, int startCol)
   {
      ...
      world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c)] = getTerrainWithName("TER_$Dry_grass").getValue();
      r += dr;
      c += dc;
      String terrName = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c)])).getName().toLowerCase();
      int pathLength = 0;
      while(!terrName.contains("water") && !terrName.startsWith("loc_") && (terrName.contains("_i_") || terrName.contains("bog") || terrName.contains("dense") || terrName.contains("mountain")))
      {
         if(terrName.contains("mountain"))
         {
            world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c)] = getTerrainWithName("TER_S_$Hills").getValue();
         }
         else if(terrName.contains("dense"))
         {
            world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c)] = getTerrainWithName("TER_$High_grassland").getValue();
         }
         else
         {
            world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c)] = getTerrainWithName("TER_$Dry_grass").getValue();
         ...
         c += dc;
         pathLength++;
         if((type.equals("temple") || type.equals("mine") || type.equals("lair") || type.equals("cave")) && pathLength > 5)
         {
            break;
         }
         ...
         terrName = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c)])).getName().toLowerCase();
      }
   }
   
   //for habitable locations, place some bridges to nearby land if they are next to water
   public static void placeBridge(byte[][]world, int r, int c)
   {
      ArrayList <Byte> bridgeDirs = new ArrayList();
      ArrayList <Coord> waterCoords = new ArrayList();
      String terrName1 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r-1)][CultimaPanel.equalizeCol(c)])).getName().toLowerCase();
      String terrName2 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r-2)][CultimaPanel.equalizeCol(c)])).getName().toLowerCase();
      if(terrName1.contains("water") || terrName1.contains("bog") || terrName1.contains("lava"))
      {
         bridgeDirs.add(NORTH);
         waterCoords.add(new Coord(CultimaPanel.equalizeRow(r-1), CultimaPanel.equalizeCol(c)));
      }
      else if(terrName2.contains("water") || terrName2.contains("bog") || terrName2.contains("lava"))
      {
         bridgeDirs.add(NORTH);
         waterCoords.add(new Coord(CultimaPanel.equalizeRow(r-2), CultimaPanel.equalizeCol(c)));
      }
      terrName1 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r+1)][CultimaPanel.equalizeCol(c)])).getName().toLowerCase();
      terrName2 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r+2)][CultimaPanel.equalizeCol(c)])).getName().toLowerCase();
      if(terrName1.contains("water") || terrName1.contains("bog") || terrName1.contains("lava"))
      {
         bridgeDirs.add(SOUTH);
         waterCoords.add(new Coord(CultimaPanel.equalizeRow(r+1), CultimaPanel.equalizeCol(c)));
      }
      else if(terrName2.contains("water") || terrName2.contains("bog") || terrName2.contains("lava"))
      {
      ...
      terrName1 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c-1)])).getName().toLowerCase();
      terrName2 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c-2)])).getName().toLowerCase();
      if(terrName1.contains("water") || terrName1.contains("bog") || terrName1.contains("lava"))
      {
         bridgeDirs.add(WEST);
         waterCoords.add(new Coord(CultimaPanel.equalizeRow(r), CultimaPanel.equalizeCol(c-1)));
      }
      else if(terrName2.contains("water") || terrName2.contains("bog") || terrName2.contains("lava"))
      {
         bridgeDirs.add(WEST);
         waterCoords.add(new Coord(CultimaPanel.equalizeRow(r), CultimaPanel.equalizeCol(c-2)));
      }
      terrName1 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c+1)])).getName().toLowerCase();
      terrName2 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(r)][CultimaPanel.equalizeCol(c+2)])).getName().toLowerCase();
      if(terrName1.contains("water") || terrName1.contains("bog") || terrName1.contains("lava"))
      ...
      if(waterCoords.size() > 0)
      {
         int numBridges = Utilities.rollDie(1, waterCoords.size());
         for(int i=0; i<numBridges && bridgeDirs.size() > 0 && waterCoords.size() > 0; i++)
         {
            int bridgeIndex = (int)(Math.random()*waterCoords.size());
            byte bridgeDir = bridgeDirs.remove(bridgeIndex);
            Coord waterStart = waterCoords.remove(bridgeIndex); 
            int dr=0, dc=0;
            if(bridgeDir==NORTH)
            {
               dr = -1;
            }
            else if(bridgeDir==EAST)
            {
               dc = 1;
            }
            else if(bridgeDir==SOUTH)
            {
               dr = 1;
            }  
            else //if(bridgeDir==WEST)
            {
               dc = -1;
            }
            int waterRow = waterStart.getRow();
            int waterCol = waterStart.getCol();
            int bridgeLength = 0;
            boolean goodBridge = false;
            terrName1 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(waterRow)][CultimaPanel.equalizeCol(waterCol)])).getName().toLowerCase();
            while(terrName1.contains("water") || terrName1.contains("bog") || terrName1.contains("lava"))
            {
               waterRow += dr;
               waterCol += dc;
               bridgeLength++;
            ...
            if(goodBridge)
            {
               waterRow = waterStart.getRow();
               waterCol = waterStart.getCol();
               terrName1 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(waterRow)][CultimaPanel.equalizeCol(waterCol)])).getName().toLowerCase();
               while(terrName1.contains("water") || terrName1.contains("bog") || terrName1.contains("lava"))
               {
                  world[CultimaPanel.equalizeRow(waterRow)][CultimaPanel.equalizeCol(waterCol)] = getTerrainWithName("INR_$Wood_Plank_floor").getValue();
                  waterRow += dr;
                  waterCol += dc;
                  terrName1 = CultimaPanel.allTerrain.get(Math.abs(world[CultimaPanel.equalizeRow(waterRow)][CultimaPanel.equalizeCol(waterCol)])).getName().toLowerCase();
               }
      ...
      for(int r=0; r<occupants.length; r++)
         for(int c=0; c<occupants[0].length; c++)
            if(occupants[r][c] != null)
            {
               ...
               if(toAdd.getCharIndex()==NPC.BEGGER)
               {
   ...
   public static Location closeLairWithPrisoners(ArrayList<Location> locations)
   {
      ...
      for(Location loc:locations)
      {
         ...
         if(!currTerrName.contains("lair") || LocationBuilder.countPrisoners(loc)<=0)
            continue;
         float distToLoc = Display.wrapDistance(r, c, CultimaPanel.player.getWorldRow(), CultimaPanel.player.getWorldCol());
         boolean added = false;
         for(int i=0; i<temp.size(); i++)
         {
            int tr = temp.get(i).getRow();
            int tc = temp.get(i).getCol();
            float distToTemp = Display.wrapDistance(tr, tc, CultimaPanel.player.getWorldRow(), CultimaPanel.player.getWorldCol());
...
//if forcePuzzle is true, it will try to build a puzzle into the location (for teach puzzle missions)
 ...
 //returns true if the location has a coffin (for SLAY_VAMPIRE mission)
  ...
  //returns true if the location has a haunted house (for HAUNTED_HOUSE mission)
   ...
   //returns true if the location has a haunted house with a bed (for HAUNTED_DARE mission)
   public static boolean hasHauntedHouseWithBed(Location loc)
   {
      byte[][]cave = null;
      if(loc.getMapIndex() >= 0 && loc.getMapIndex() < CultimaPanel.map.size())
         cave=CultimaPanel.map.get(loc.getMapIndex());
         ...
         for(int c=1; c<cave[r].length-1; c++)
         {
            ...
            if(current.contains("gray_tile_wall"))
            {
               for(int rr=r-1; rr<=r+1; rr++)
               {
                  ...
                  {
                     if(rr<0 || cc<0 || rr>=cave.length || cc>=cave[0].length)
                        continue;
                     String bed = CultimaPanel.allTerrain.get(Math.abs(cave[rr][cc])).getName().toLowerCase();
                     if(bed.contains("bed"))
                     {
                        return true;
                     }
         ...
         || (locName.contains("temple") && Math.random() < 0.5)  || (locName.contains("mine") && Math.random() < 0.25)) 
         {
            ...
            for(int j=1; j<CultimaPanel.allLocations.size(); j++)
            {
               ...
               //if(target.getName().contains("east") || target.getName().contains("west") || (i==j))
                  //continue;
                  float distance = Display.findDistance(loc.getRow(), loc.getCol(), target.getRow(), target.getCol());
...
//returns the direction that would get (r1,c1) closer to (r2,c2)
   ...
   {
      float distWrap = Display.wrapDistance(r1, c1, r2, c2);
