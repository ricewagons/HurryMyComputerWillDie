...
//components for giving game elements random names
   ...
   //returns a random byte between [min and max] inclusive
   public static byte rollDie(byte min, byte max)
   {
...
//picks a lock next to row, col
   public static boolean pickALock(byte[][]cave, int row, int col)
   {
      ...
      boolean isCage = (locType.contains("camp") || thisLocName.endsWith(" camp") || CultimaPanel.player.getLocationType().toLowerCase().contains("camp"));
               ...
               if(index > 0)   //destroy trap, or change locked door to unlocked
               {
                  ...
                  for(Terrain t: CultimaPanel.allTerrain)
                  {
                     ...
                     if(descr.contains(temp) && !descr.contains("locked") && !descr.contains("trap") && !descr.contains("book"))
                     {
                        ...
                        success = true;
                        if(!isCage)
                        {  //check to see if the door is an iron bar door and next to the red ship wall
                           isCage |= (name.toLowerCase().contains("iron_bar_door") && nextToTerrainWithName(cave, r, c, "red_ship_wall"));
                        }
                        if(isCage)
                        {  //if this is unlocking the cell of a trapped creature in a camp, set its move-type or alignment
                           ...
                           Object[] adjacentInfo = getAdjacentNPCsWithIndex(r, c, CultimaPanel.player.getMapIndex());
                           NPCPlayer[] adjacents = (NPCPlayer[])(adjacentInfo[0]);
                           int[] indecies = (int[])(adjacentInfo[1]);
                           String[] collections = (String[])(adjacentInfo[2]);
                           if(adjacents != null)
                           {
                              for(int i=0; i<adjacents.length; i++)
                              {
                                 ...
                                 //once the iron door is unlocked, monster will spring out, removing the door themselves
                                    ...
                                    else if(pType==NPC.PANGOLIN)
                                    {
                                       int pIndex = indecies[i];
                                       String pCollectionType = collections[i];
                                       if(pCollectionType!=null && pCollectionType.contains("civilians"))
                                       {
                                          if(pIndex >=0 && pIndex < CultimaPanel.civilians.get(mi).size())
                                          {
                                             NPCPlayer pTemp = CultimaPanel.civilians.get(mi).get(pIndex);
                                             if(pTemp.equals(p))
                                             {  //see if we have a PANGOLIN_PROTECT mission active and are in the mission location
                                                //If so, the mission fails and we lose reputation points
                                                for(int j=CultimaPanel.missionStack.size()-1; j>=0; j--)
                                                {
                                                   Mission m = CultimaPanel.missionStack.get(j);
                                                   int mRow = m.getWorldRow(), mCol = m.getWorldCol();
                                                   int plRow = CultimaPanel.player.getWorldRow(), plCol = CultimaPanel.player.getWorldCol();
                                                   if(m.getMissionType()==Mission.PANGOLIN_PROTECT && mRow==plRow && mCol==plCol)
                                                   {
                                                      CultimaPanel.missionStack.remove(j);
                                                      CultimaPanel.sendMessage("-----");
                                                      CultimaPanel.sendMessage("Pangolin mission failed!");
                                                      CultimaPanel.player.addReputation(-5);
                                                      break;
                                                   }
                                                }
                                             
                                                NPCPlayer roller = new NPCPlayer(NPC.ROLLINGMENACE, p.getRow(), p.getCol(), p.getMapIndex(), locType);
                                                roller.addItems(p.getItems());
                                                roller.setGold(p.getGold());
                                                roller.setNumArrows(p.getNumArrows());
                                                roller.setMoveType(Utilities.rollDie(NPC.MARCH_NORTH, NPC.MARCH_WEST));
                                                Sound.rumble();
                                                CultimaPanel.civilians.get(mi).set(pIndex, roller);
                                                CultimaPanel.player.addExperience(10); //the gods are pleased
                                                CultimaPanel.player.addReputation(10);
                                             }
                                          }
                                       }
                                       else if(pCollectionType!=null && pCollectionType.contains("worldMonsters"))
                                       {
                                          if(pIndex >=0 && pIndex < CultimaPanel.worldMonsters.size())
                                          {
                                             NPCPlayer pTemp = CultimaPanel.worldMonsters.get(pIndex);
                                             if(pTemp.equals(p))
                                             {
                                                NPCPlayer roller = new NPCPlayer(NPC.ROLLINGMENACE, p.getRow(), p.getCol(), p.getMapIndex(), locType);
                                                roller.addItems(p.getItems());
                                                ...
                                                roller.setMoveType(Utilities.rollDie(NPC.MARCH_NORTH, NPC.MARCH_WEST));
                                                Sound.rumble();
                                                CultimaPanel.worldMonsters.set(pIndex, roller);
                                             }
                                          }
                                       }
                                    }
  ...
  //sees if we are next to any NPCs that we can pickpocket 
   public static void getAdjacentNPCs()
   {
      ...
      if(mi == 0)
      {
         for(int pi=CultimaPanel.boats.size()-1; pi>=0; pi--)
         {
            ...
            float dist = Display.findDistance(p.getRow(), p.getCol(), r, c);
      ...
      for(int pi=CultimaPanel.civilians.get(mi).size()-1; pi>=0; pi--)
      {
         ...
         if(hasKnowing)
         {
            ...
            {
               float distToLastTarg = Float.MAX_VALUE;
   ...
   //returns any adjacent npcs that are next to r,c with 3 return values in an array:
   //index 0 is the collection of NPCPlayers for each of the 4 directions
   //index 1 is the index in which they were found in a collection
   //index 2 is the name of the collection it was found in
   public static Object[] getAdjacentNPCsWithIndex(int r, int c, int mi)
   {
      ...
      {   
         for(int i=0; i<CultimaPanel.boats.size(); i++)
         {
            ...
            {
               adjacent[LocationBuilder.NORTH] = p;
               indecies[LocationBuilder.NORTH] = i;
               collectionName[LocationBuilder.NORTH] = "boats";
            }
            if(p.getRow()==r+1 && p.getCol()==c)
            {
               adjacent[LocationBuilder.SOUTH] = p;
               indecies[LocationBuilder.SOUTH] = i;
               collectionName[LocationBuilder.SOUTH] = "boats";
            }
            if(p.getRow()==r && p.getCol()==c-1)
            {
               adjacent[LocationBuilder.WEST] = p;
               indecies[LocationBuilder.WEST] = i;
               collectionName[LocationBuilder.WEST] = "boats";
            }
            if(p.getRow()==r && p.getCol()==c+1)
            {
               adjacent[LocationBuilder.EAST] = p;
               indecies[LocationBuilder.EAST] = i;
               collectionName[LocationBuilder.EAST] = "boats";
            }
         } 
      }     
      for(int i=0; i<CultimaPanel.civilians.get(mi).size(); i++)
      {
         ...
         if(p.getRow()==r-1 && p.getCol()==c)
         {
            adjacent[LocationBuilder.NORTH] = p;
            indecies[LocationBuilder.NORTH] = i;
            collectionName[LocationBuilder.NORTH] = "civilians";
         }
         if(p.getRow()==r+1 && p.getCol()==c)
         {
            adjacent[LocationBuilder.SOUTH] = p;
            indecies[LocationBuilder.SOUTH] = i;
            collectionName[LocationBuilder.SOUTH] = "civilians";
         }
         if(p.getRow()==r && p.getCol()==c-1)
         {
            adjacent[LocationBuilder.WEST] = p;
            indecies[LocationBuilder.WEST] = i;
            collectionName[LocationBuilder.WEST] = "civilians";
         }
         if(p.getRow()==r && p.getCol()==c+1)
         {
            adjacent[LocationBuilder.EAST] = p;
            indecies[LocationBuilder.EAST] = i;
            collectionName[LocationBuilder.EAST] = "civilians";
         }
      }     
      for(int i=0; i<CultimaPanel.worldMonsters.size(); i++)
      {
         ...
         if(p.getRow()==r-1 && p.getCol()==c && p.getMapIndex()==mi)
         {
            adjacent[LocationBuilder.NORTH] = p;
            indecies[LocationBuilder.NORTH] = i;
            collectionName[LocationBuilder.NORTH] = "worldMonsters";
         }
         if(p.getRow()==r+1 && p.getCol()==c && p.getMapIndex()==mi)
         {
            adjacent[LocationBuilder.SOUTH] = p;
            indecies[LocationBuilder.SOUTH] = i;
            collectionName[LocationBuilder.SOUTH] = "worldMonsters";
         }
         if(p.getRow()==r && p.getCol()==c-1 && p.getMapIndex()==mi)
         {
            adjacent[LocationBuilder.WEST] = p;
            indecies[LocationBuilder.WEST] = i;
            collectionName[LocationBuilder.WEST] = "worldMonsters";
         }
         if(p.getRow()==r && p.getCol()==c+1 && p.getMapIndex()==mi)
         {
            adjacent[LocationBuilder.EAST] = p;
            indecies[LocationBuilder.EAST] = i;
            collectionName[LocationBuilder.EAST] = "worldMonsters";
         }
      }  
      for(int i=0; i < CultimaPanel.furniture.get(mi).size(); i++)
      {
         ...
         if(p.getRow()==r-1 && p.getCol()==c)
         {
            adjacent[LocationBuilder.NORTH] = p;
            indecies[LocationBuilder.NORTH] = i;
            collectionName[LocationBuilder.NORTH] = "furniture";
         }
         if(p.getRow()==r+1 && p.getCol()==c)
         {
            adjacent[LocationBuilder.SOUTH] = p;
            indecies[LocationBuilder.SOUTH] = i;
            collectionName[LocationBuilder.SOUTH] = "furniture";
         }
         if(p.getRow()==r && p.getCol()==c-1)
         {
            adjacent[LocationBuilder.WEST] = p;
            indecies[LocationBuilder.WEST] = i;
            collectionName[LocationBuilder.WEST] = "furniture";
         }
         if(p.getRow()==r && p.getCol()==c+1)
         {
            adjacent[LocationBuilder.EAST] = p;
            indecies[LocationBuilder.EAST] = i;
            collectionName[LocationBuilder.EAST] = "furniture";
         }
      }
      retVal[0] = adjacent;
      retVal[1] = indecies;
      retVal[2] = collectionName;   
      return retVal;
   }
   ...
   public static ArrayList<NPCPlayer> adjacentHurtNPCs(int range, boolean onlyPoisoned)
   {
      ...
      for(NPCPlayer p:CultimaPanel.civilians.get(mi))
      {
         ...
         {
            float distToPlayer = Display.findDistance(p.getRow(), p.getCol(), r, c);
   ...
   //true if (row,col) is next to terrain that contains a certain name. 
   public static boolean nextToTerrainWithName(byte[][]currMap, int row, int col, String name)
   {
      String current = "";
      if(row+1>=0 && row+1<currMap.length && col>=0 && col<currMap[0].length)
      {
...
//returns true if cave[r][c] is next to a lock to pick
  ...
  //used to see if we are next to a wood door - for npc werewolf breaking down
   ...
   public static boolean nextToLiquidLeft()
   {
      int r=CultimaPanel.player.getRow();
      int c=CultimaPanel.player.getCol();
      int mi=CultimaPanel.player.getMapIndex();  
      if(r < 0 || c < 0 || mi<0 || mi>CultimaPanel.map.size())
         return false;
      byte[][]currMap = CultimaPanel.map.get(mi);
      if (c-1>=0 && CultimaPanel.allTerrain.get(Math.abs(currMap[r][c-1])).isLiquid() && !CultimaPanel.allTerrain.get(Math.abs(currMap[r][c])).isLiquid() && !CultimaPanel.allTerrain.get(Math.abs(currMap[r][c])).isMoving())
         return true;
      return false;
   }
   
   public static boolean nextToLiquidRight()
   {
      int r=CultimaPanel.player.getRow();
      int c=CultimaPanel.player.getCol();
      int mi=CultimaPanel.player.getMapIndex();  
      if(r < 0 || c < 0 || mi<0 || mi>CultimaPanel.map.size())
         return false;
      byte[][]currMap = CultimaPanel.map.get(mi);
      if (c+1<currMap[0].length && CultimaPanel.allTerrain.get(Math.abs(currMap[r][c+1])).isLiquid() && !CultimaPanel.allTerrain.get(Math.abs(currMap[r][c])).isLiquid() && !CultimaPanel.allTerrain.get(Math.abs(currMap[r][c])).isMoving())
         return true;
      return false;
   }
...
//returns true if the player can break a wall they are next to
   public static boolean canBreakWall()
   {
      ...
      boolean cutOutOfBeast = ((p.getLocationType().contains("graboid") || p.getLocationType().contains("beast")) &&  //we can cut our way out of a graboid's innnards
      ...
      if(flight || armName.contains("cloak") || p.getLocationType().contains("pacrealm") || p.getLocationType().contains("frogrealm"))
         return false;
      if(p.getMapIndex()==0)
         return false;
      ...
      {
         float d1dist = Display.distanceSimple((int)(CultimaPanel.door1.getRow()),(int)(CultimaPanel.door1.getCol()),p.getRow(),p.getCol());
         float d2dist = Display.distanceSimple((int)(CultimaPanel.door2.getRow()),(int)(CultimaPanel.door2.getCol()),p.getRow(),p.getCol());
         float d3dist = Display.distanceSimple((int)(CultimaPanel.door3.getRow()),(int)(CultimaPanel.door3.getCol()),p.getRow(),p.getCol());
   ...
   public static boolean canSetOnFire()
   {
      ...
      String armName = CultimaPanel.player.getArmor().getName().toLowerCase();
      Player p = CultimaPanel.player;
      if(flight || armName.contains("invisibility-cloak") || CultimaPanel.player.isWolfForm() || CultimaPanel.player.isSailing() || p.getLocationType().contains("pacrealm") || p.getLocationType().contains("frogrealm"))
         return false; 
...
//set fires for defensive fireshield spell
      ...
      for(int r=row-(range+2); r<=row+(range+2); r++)
      {
         for(int c=col-(range+2); c<=col+(range+2); c++)
         {
            ...
            if(target != null && target.getBody() > 0)
            {
               ...
               CultimaPanel.smoke.add(new SmokePuff(target.getNpcX(), target.getNpcY(), radius, opacity, fire));
            }
         }
      }
  ...
  //post: returns true if p is in range for us to attack
   ...
   { 
      if(p.getLocationType().contains("frogrealm"))
         return false;
      ...
      float dist = Display.findDistance(p.getRow(), p.getCol(), row, col);
...
//returns true if there is a vision blocking Terrain between (r1,c1) and (r2, c2) - assuming they are in the same row or col (for attacking)
   ...
   public static float distToClosestPrisoner()
   {
      ...
      for(NPCPlayer p:CultimaPanel.humanNPCsInOurMap)
      {
         ...
         { 
            ...
            {  
               float thisDist = Display.distanceSimple(p.getRow(), p.getCol(), CultimaPanel.player.getRow(), CultimaPanel.player.getCol());
   ...
   public static float distToBarkeep()
   {
      float dist = Float.MAX_VALUE;
      for(NPCPlayer p:CultimaPanel.humanNPCsInOurMap)
      {
  ...
  //This will also return the distance to a shopkeep so that they can make a comment about their wares when you get close
   ...
   {
      float distToBegger = Float.MAX_VALUE;
      float distToPoisoned = Float.MAX_VALUE;
      int reputation = 0;
      String npcName = "";
   ...
   //when we enter a location, check to see if there is a ceremony mission here and if so, activate it
   public static void checkCeremonyMission()
   {
      if(CultimaPanel.numChants != -1)
         return;
      for(int i=CultimaPanel.missionStack.size()-1; i>=0; i--)
      {
         Mission m = CultimaPanel.missionStack.get(i);
         int mWorldRow = m.getTargetRow();
         int mWorldCol = m.getTargetCol();
         if(mWorldRow == -1 && mWorldCol == -1)
         {
            mWorldRow = m.getWorldRow();
            mWorldCol = m.getWorldCol();
         }
         int pWorldRow = CultimaPanel.player.getWorldRow();
         int pWorldCol = CultimaPanel.player.getWorldCol();
         if(m.getType().contains("Ceremony") && pWorldRow==mWorldRow && pWorldCol==mWorldCol && m.getDay()==-1)
         {
            CultimaPanel.numChants = 0;
            m.setDay(Integer.MAX_VALUE);
         }
      }
   }
...
//returns a collection of all non-statue, non-zombie npcs at a certain mapindex for selecting mission targets
   ...
   public static void rollerMove(NPCPlayer p)
   {
      if(CultimaPanel.numFrames % 4 != 0)
         return;
      int mi = p.getMapIndex();
      byte[][]currMap = (CultimaPanel.map.get(mi));   
      boolean inHabitable = TerrainBuilder.habitablePlace(p.getLocationType());
      int r = p.getRow();
      int c = p.getCol();
      int dr = 0;
      int dc = 0;    
      NPCPlayer[] adjacents = getAdjacentNPCs(r, c, mi);
      ...
      String goingToSpot = "";
      if(!LocationBuilder.outOfBounds(currMap, r+dr, c+dc))
      {
         goingToSpot = CultimaPanel.allTerrain.get(Math.abs(currMap[r+dr][c+dc])).getName().toLowerCase();
      }
      boolean badSpot = (goingToSpot.contains("iron") || (goingToSpot.contains("_i_")  && (goingToSpot.contains("earth") || goingToSpot.contains("rock"))));
      if(inHabitable)
      {  //if in a habitable place, these will not go through iron doors but everything else
         badSpot = (goingToSpot.contains("iron"));
      }
      if((CultimaPanel.player.getRow()==r+dr && CultimaPanel.player.getCol()==c+dc))
      {  //moving into player - damage player
         p.attack();
      }
      for(NPCPlayer target: adjacents)
      {
         if(target != null && target.getCharIndex()!=NPC.ROLLINGMENACE && target.getCharIndex()!=NPC.PANGOLIN)
         {
            p.attack(target);
         }
      }  
      if(badSpot)
      {
         if(p.getMoveType()==NPC.MARCH_NORTH || p.getMoveType()==NPC.MARCH_SOUTH)
         {
            if(Math.random() < 0.5f)
            {
...
//NPC Players will more according to move type STILL, ROAM, MARCH, CHASE or RUN and their agility compared to Player
   public static void moveNPCs(boolean keyHit)
   {
      ...
      else if(locType.contains("frogrealm"))
      {
         frogRealmMovement();
      }   
      else
      {
         ...
         CultimaPanel.distToLute = Float.MAX_VALUE;
         for(int i=civsInLoc.size()-1; i>=0; i--)
         {
            NPCPlayer p = civsInLoc.get(i);   
         
            if(p.getBody() <= 0 && p.getCharIndex() != NPC.ROLLINGMENACE)
               continue;
            ...
            if(curr.contains("poison") && !Display.isWinter() && !p.hasEffect("poison") && !p.canFly() && Math.random() < 0.1 && p.getCharIndex()!=NPC.RAT && p.getCharIndex()!=NPC.RATKING)
            {  //if p is standing in a poison bog, chance they will be poisoned
               p.addEffect("poison");
            }
         
            if(p.hasEffect("still") || p.hasEffect("web") || (p.getCharIndex()==NPC.LANDSHARK && p.hasEffect("stun")))
               continue; 
            ...
            if(specialMove(p, i, civsInLoc))
            {  //see if npc uses this turn for a special move like summoning or a special attack
               continue;
            }
            float distToPlayer = Display.findDistance(p.getRow(), p.getCol(), player.getRow(), player.getCol());
            ...
            if(p.getMoveType()==NPC.RUN && distToPlayer > (CultimaPanel.SCREEN_SIZE/2+1))
               continue;   
            if(p.getCharIndex() != NPC.ROLLINGMENACE && (guardDefender || p.getMoveType()==NPC.ROAM || (p.getMoveType()>=NPC.MARCH_NORTH  && p.getMoveType()<=NPC.MARCH_WEST) || player.isInvisible()))
            {
         ...
         //a numInfo of 13 is the tag that a guard has been bribed
            ...
            if(p.getCharIndex() == NPC.ROLLINGMENACE)
            {
               rollerMove(p);
               continue;
            }
            ...
            float distToTarg = Float.MAX_VALUE;
            if(target != null)
            {
            ...
            if(p.getCharIndex() == NPC.GORGON)
            {
               if( checkGorgonAttack(p))
               {
                  continue;
               }
            }
            ...
            else if(p.getMoveType()==NPC.CHASE || p.getMoveType()==NPC.RUN  || p.getMoveType()==NPC.FOLLOW)
            {
               ...
               {  //if we are hiding, make it so that the creature seeking us essentially is put to roam depending on how agilities compare
                  ...
                  float agilDiff = Math.min(hideValue, (CultimaPanel.player.getAgility() / (Math.max(1, p.getAgility()))));
   ...
   public static boolean checkGorgonAttack(NPCPlayer p)
   {
      if(p.getCharIndex() == NPC.GORGON && p.getNumInfo()>=4 && System.currentTimeMillis() > CultimaPanel.alertTime)
      {  //10-second cooldown between stonecast for the gorgon
         CultimaPanel.alertTime = System.currentTimeMillis() + 10000;
         p.setNumInfo(0);
         Sound.explosion();
         CultimaPanel.flashColor = Color.cyan;
         CultimaPanel.flashFrame = CultimaPanel.numFrames;
         CultimaPanel.flashRadius = CultimaPanel.SCREEN_SIZE;
         //see if the gorgon is in our view, we get hit with the stonecast
         boolean inOurSight = false;
         for(NPCPlayer curr:CultimaPanel.AllNPCinSight)
         {
            if(curr.getCharIndex() == NPC.GORGON)
            {
               inOurSight = true;
               break;
            }
         }
         boolean reflected = (CultimaPanel.player.getWeapon().getName().toLowerCase().contains("mirror")) && (CultimaPanel.player.isOnGuard() || Math.random() < 0.25); 
         if(inOurSight)
         {
            if(reflected)
            {
               Sound.trapSprung();
               CultimaPanel.sendMessage("-----");
               CultimaPanel.sendMessage("Thy weapon reflects the Gorgon's gaze!");
            }
            else
            {
               CultimaPanel.player.damage(CultimaPanel.player.getBody());
               CultimaPanel.sendMessage("-----");
               CultimaPanel.sendMessage("Thy flesh turns to stone!");
               CultimaPanel.player.removeAllItemsWithName("life-pearl");
            }
            return true;
         } 
      }
   ...
   {
      String [] gorgon = {"The Gorgon's eyes open.","The Gorgon's eyes brighten!","The Gorgon's eyes are blinding!"};
      float dist = Display.findDistance(p.getRow(), p.getCol(), CultimaPanel.player.getRow(), CultimaPanel.player.getCol());
      if((p.getCharIndex()==NPC.BEHOLDER || p.getCharIndex()==NPC.LICHE) && CultimaPanel.numFrames%350==0)
      {
         int psyAttack = Utilities.rollDie(0,2);
         p.setNumInfo(psyAttack);
         if(psyAttack == 1)
         {
            CultimaPanel.sendMessage("A weight is cast on thy mental energy!");
         }
         else if(psyAttack == 2)
         {
            CultimaPanel.sendMessage("Thy very life force is being slowly pulled away!");
         }
         else //if(psyAttack == 0)
         {
            CultimaPanel.sendMessage("The "+NPC.characterDescription(p.getCharIndex())+" pauses to charge its psionic attack...");
         }
      }
      else if(p.getCharIndex()==NPC.EARTHELEMENTAL && p.getNumInfo()>0 && Math.random() < 0.025 && dist <= 2)    
      {  //EARTHELEMENTAL might surround you with raised stones
         p.setNumInfo(p.getNumInfo()-1);
         surroundWithStones();
         return true;
      }
      else if(p.getCharIndex()==NPC.GORGON && System.currentTimeMillis() > CultimaPanel.alertTime + 2000 && p.getNumInfo() < 4)
      {  //GORGON will charge up Stone-cast spell (Three 1-second charge-ups before firing)
         CultimaPanel.alertTime = System.currentTimeMillis();
         if(p.getNumInfo()>=0 && p.getNumInfo()<gorgon.length)
         {
            CultimaPanel.sendMessage("-----");
            CultimaPanel.sendMessage("<"+(3-p.getNumInfo())+"> "+gorgon[p.getNumInfo()]);
         }
         p.setNumInfo(p.getNumInfo()+1);
         Sound.chargeUp(p.getNumInfo());
      }
      else if(p.getCharIndex()==NPC.DEMONKING && p.getNumInfo()>0 && Math.random() < 0.025 && dist <= CultimaPanel.SCREEN_SIZE/2 && !p.getName().toLowerCase().contains("puzzle"))    
      {  //DEMONKING might summon a demon minion on this movement
         ...
         else
         {
            ...
            if(success)
            {
               ...
               {
                  CultimaPanel.sendMessage("-----");
                  CultimaPanel.sendMessage("~Vocare Ludicro!");
                  CultimaPanel.sendMessage("Mimic summoned!");
               }
               else
               {
                  CultimaPanel.sendMessage("-----");
                  CultimaPanel.sendMessage("~Daemonem Arcesse!");
      ...
      else if(p.getCharIndex()==NPC.MAGMA && p.getNumInfo()>0 && Math.random() < 0.025 && dist <= CultimaPanel.SCREEN_SIZE/2)    
      ...
      else if(p.getCharIndex()==NPC.FIRE && p.getNumInfo()>1 && Math.random() < 0.02 && dist <= CultimaPanel.SCREEN_SIZE/2)    
      ...
      else if(p.getCharIndex()==NPC.PANGOLIN && p.hasBeenAttacked())
      {
         NPCPlayer roller = new NPCPlayer(NPC.ROLLINGMENACE, p.getRow(), p.getCol(), p.getMapIndex(), CultimaPanel.player.getLocationType());
         roller.addItems(p.getItems());
         roller.setGold(p.getGold());
         roller.setNumArrows(p.getNumArrows());
         roller.setMoveType(Utilities.rollDie(NPC.MARCH_NORTH, NPC.MARCH_WEST));
         ...
         {
            npcList.set(i, roller);
         }
      }
      else if(NPC.isVampire(p.getCharIndex()))
      {
         ...
         {  //change to bats and run when hurt
            ...
            bat.setGold(p.getGold());
            bat.setNumArrows(p.getNumArrows());
            if(i>=0 && i<npcList.size())
            {
               npcList.set(i, bat);
            }
      ...
      else if(p.getCharIndex()==NPC.TRIFFID && Math.random() < 0.02 && dist <= CultimaPanel.SCREEN_SIZE/8)    
   ...
   public static void affectTerrain(NPCPlayer p)
   {
      ...
      if((NPC.isGiantMonsterKing(p) && TerrainBuilder.habitablePlace(p.getLocationType())) || p.getCharIndex()==NPC.ROLLINGMENACE)
         ...
         for(int mr=-1; mr<=1; mr++)
         {
            for(int mc=-1; mc<=1; mc++)
            {
               if(p.getRow() + mr >= 0 && p.getRow() + mr < currMap.length && p.getCol() + mc >=0 && p.getCol() + mc < currMap[0].length)
               {
                  ...
                  if(p.getCharIndex()==NPC.ROLLINGMENACE)
                  {
                     if(closeSpot.contains("iron") || mi == 0)
                        continue;
                  }
                  ...
                  /*else if(p.getCharIndex()!=NPC.ROLLINGMENACE && ((closeSpot.contains("floor") && !closeSpot.contains("black") ) || (closeSpot.contains("brick") && !closeSpot.contains("_i_"))))
                  {
                  ...
                  else if(!closeSpot.contains("floor") && (closeSpot.contains("inr_") || closeSpot.contains("wall") || closeSpot.contains("door") || closeSpot.contains("rock") || closeSpot.contains("torch") || closeSpot.contains("window") || (closeSpot.contains("brick") && closeSpot.contains("_i_"))))
                  {
                     byte wallValue = (byte)(Math.abs(currMap[p.getRow()+mr][p.getCol()+mc]));
      ...
      for(int i=civsInLoc.size()-1; i>=0; i--)
      {
         ...
         if(p.getCharIndex()==NPC.MAZEMOUTH)
         {
            ...
            for(int j=(CultimaPanel.civilians.get(mi)).size()-1; j>=0; j--)
            {
               ...
               float distToNPC = Display.distanceSimple(p.getRow(), p.getCol(), currNPC.getRow(), currNPC.getCol());
         ...
         float distToPlayer = Display.distanceSimple(p.getRow(), p.getCol(), CultimaPanel.player.getRow(), CultimaPanel.player.getCol());
         if(!ghostFear)
         {
   ...
   public static void frogRealmMovement()
   {
      int mi = CultimaPanel.player.getMapIndex();
      ArrayList<NPCPlayer> civsInLoc = (CultimaPanel.civilians.get(mi));
      for(int i=civsInLoc.size()-1; i>=0; i--)
      {
         ...
         if(p==null || !p.getLocationType().contains("frogrealm") || p.getCharIndex()==-111)
            continue;
         int r = p.getRow();
         int c = p.getCol();
         if(mi < 0 || mi >= CultimaPanel.map.size() || r < 0 || c < 0)
         ...
         if(p.getCol()==0)
         {
            p.setCol(currMap[0].length-2);
            continue;
         }
         else if(p.getCol()==currMap[0].length-1)
         {
            p.setCol(1);
            continue;
         }   
         int delayTime = (int)(CultimaPanel.animDelay);
         float distToPlayer = Display.distanceSimple(p.getRow(), p.getCol(), CultimaPanel.player.getRow(), CultimaPanel.player.getCol());
         if(distToPlayer <= 1 && CultimaPanel.player.getBody() > 0 && p.getRow()==CultimaPanel.player.getRow())
         { 
            p.attack();
         }
         if(CultimaPanel.numFrames % delayTime != 0)  
   ...
   public static boolean setMovementType(NPCPlayer p, boolean searchMessage, boolean guardDefender)
   {
         ...
         for(int mIndex=CultimaPanel.missionStack.size()-1; mIndex>=0; mIndex--)
         {
            ...
            if(currMission.getMissionType()==Mission.KAIJU)
            {
               ...
               if(cityLoc != null)
               {
                  ...
                  {
                     float distToCity = Display.findDistance(kaijuLocation.getRow(), kaijuLocation.getCol(), cityLoc.getRow(), cityLoc.getCol());
      ...
      if(p.getCharIndex()==NPC.MINDDEVOURER && !p.hasBeenAttacked() && p.getMoveType()!=NPC.RUN)
      {
         int playerMind = CultimaPanel.player.getMind();
         if(playerMind <= 15)
         {
            p.setMoveType(NPC.ROAM);
         }
         else if(playerMind > 15 && playerMind <= 25 && dist <= 2)
         {
            p.setMoveType(NPC.CHASE);
         }
         else if(playerMind > 25 && playerMind <= 35 && dist <= 4)
         {
            p.setMoveType(NPC.CHASE);
         }
         else if(playerMind > 35 && playerMind <= 45 && dist <= 6)
         {
            p.setMoveType(NPC.CHASE);
         }
         else if(playerMind > 45 && dist <= 8)
         {
            p.setMoveType(NPC.CHASE);
         }
         return searchMessage;
               ...
               else
               {
                  ...
                  if((System.currentTimeMillis() > CultimaPanel.cheerTime + 5000) && !searchMessage)
                  {
                     if(monster!=null)
                     {
                        ...
                        else if(monster.getCharIndex()==NPC.TROGDOR)
                        {
                           CultimaPanel.sendMessage("~"+NPC.getRandomFrom(NPC.helpTrogdor).replace("PLAYER_NAME", CultimaPanel.player.getShortName()));
                           searchMessage = true;
                        }
                        else
                        {
      ...
      float dogDist = Integer.MAX_VALUE;    //will be the distance between our player and our dog if we have one
      ...
      float smellValue = 0;  //value to modify distance needed for a monster to detect you
      ...
      if((p.getCharIndex()==NPC.UNICORN || p.getCharIndex()==NPC.PANGOLIN) && dist < 3 && CultimaPanel.player.numFlowers() < 5 && !p.hasBeenAttacked())
   ...
   public static void firestorm()
   {
      ...
      float mindPercent = (CultimaPanel.player.getMind() / 50.0f);                //the percentage we are to max mind, used to do more damage depending on level
  ...
  //a random fire starts on a combustable place on the screen (lightning strike)
         ...
         for(int r=row - 1; r<=row+1; r++)
         {
            for(int c=col - 1; c<=col + 1; c++)
            {
               ...
               {  //if there is no NPC there, add the point into firesToAdd
                  ...
                  float fireSpreadProb = 0.05f;
                  if(CultimaPanel.windDir == LocationBuilder.NORTH)
                  {
                     if(checkRow < row)
                        fireSpreadProb = 0.06f;
                     else if(checkRow == row)
                        fireSpreadProb = 0.04f;
                     else
                        fireSpreadProb = 0.02f;      
                  }
   ...
   public static boolean areaOfEffectAttack(NPCPlayer target, int range, Weapon weapType)
   {
      ...
      for(int i=CultimaPanel.worldMonsters.size()-1; i>=0; i--)
      {
         ...
         float dist = Display.distanceSimple(target.getRow(), target.getCol(), p.getRow(), p.getCol());
         if(dist > range)
         { 
            continue;
         } 
         float distPercent = (range - dist) / range;
         if(range == 1 && dist == 1)
         {
            distPercent = 1;
         }
      ...
      if(weapName.contains("fire") || weapName.contains("lightning"))
      {
         ...
         for(int i=0; i< locs.size(); i++)
         {
            ...
            int r = pick.getRow();
            int c = pick.getCol();
            float dist = Display.distanceSimple(target.getRow(), target.getCol(), r, c);
            float distPercent = (range - dist) / range;
...
//see if there is a player between monolith attacker and another, and attack if so
   public static void monolithAttack(NPCPlayer attacker)
   {
      ...
      for(int i=CultimaPanel.worldMonsters.size()-1; i>=0; i--)
      {
         ...
         {
            float dist = Display.findDistance(attRow, attCol, p.getRow(), p.getCol());
            if(dist <= weaponRange)
               attacker.attack(p);
         }
   ...
   public static void raiseWater(int row, int col, int mi)
   {
      ...
      for(int r=row - 2; r<=row+2; r++)
      {
         ...
         {
            float dist = Display.distanceSimple(r, c, row, col);
   ...
   public static void planeMove(NPCPlayer p, byte[][]house)
   {
      ...
      float weaponRange = p.getWeapon().getRange();
      for(int i=CultimaPanel.worldMonsters.size()-1; i>=0; i--)
      {
...
//maybe make extra move for monsters with a higher agility if keyHit is true
   public static void moveMonsters(boolean keyHit)
   {
      ...
      for(int i=CultimaPanel.worldMonsters.size()-1; i>=0; i--)
      {
         ...
         if(p.isNonPlayer() || p.isPellet())
         {                    //WILLOWISP in pac-realm are pellets
            continue;
         }
      ...
      //ship that has been boarded by player should not move on its own   
         ...
         String playerSpot = CultimaPanel.allTerrain.get(Math.abs(currMap[CultimaPanel.player.getRow()][CultimaPanel.player.getCol()])).getName().toLowerCase();
         String pSpot = CultimaPanel.allTerrain.get(Math.abs(currMap[p.getRow()][p.getCol()])).getName().toLowerCase();
         if(pSpot.contains("poison") && !Display.isWinter() && !p.hasEffect("poison") && !p.canFly() && Math.random() < 0.1 && p.getCharIndex()!=NPC.RAT && p.getCharIndex()!=NPC.RATKING)
         {  //if p is standing in a poison bog, chance they will be poisoned
            p.addEffect("poison");
         }
         if(p.hasEffect("still") || p.hasEffect("web") || (p.getCharIndex()==NPC.LANDSHARK && p.hasEffect("stun")))
         ...
         if(p.getMoveType()==NPC.RUN && dist > (CultimaPanel.SCREEN_SIZE/2+1))
            continue;   
         if(p.getCharIndex() != NPC.ROLLINGMENACE && (p.getMoveType()==NPC.ROAM || (p.getMoveType()>=NPC.MARCH_NORTH  && p.getMoveType()<=NPC.MARCH_WEST)))
         {
         ...
         else if(p.getMoveType()==NPC.CHASE || p.getMoveType()==NPC.RUN || p.getMoveType()==NPC.ATTACK)
         {
            ...
            {  //if we are hiding, make it so that the creature seeking us essentially is put to roam depending on how agilities compare
               ...
               if(CultimaPanel.fogAlpha > 0)
               {  //more effective in fog
                  hideValue += CultimaPanel.fogAlpha/1000.0f;
               }
               if(CultimaPanel.player.hasBrightWeapon())
               {  //if we have a bright or flaming weapon out, we are not hiding very well
                  hideValue = 0.1f;
            ...
            if(locType.contains("battlefield") && target!=null)
            {
               ...
               {  //chase and attack whoever is closer: player or closest enemy NPC
                  float distToTarg = Display.findDistance(p.getRow(), p.getCol(), target.getRow(), target.getCol());
                  float distToPlayer = Display.findDistance(p.getRow(), p.getCol(), playerR, playerC);
...
//returns true if the location is a capital city
   ...
   public static Location getClosestCapital(int r, int c)
   {
      ...
      for(Location loc:CultimaPanel.allDomiciles)
         ...
         {
            float dist = Display.wrapDistance(r, c, loc.getRow(), loc.getCol());
            if(dist < minDist)
            {
               minDist = dist;
               closest = loc;
...
//place commonors out on the sreet roaming in cities (not castles)
   ...
   //rare chance of a beast running though the city
      ...
      {
         float fishProb = 0.66f;
         float sharkProb = 0.2f;
...
//if we win the 3-doors puzzle, remove the magic dragons
   ...
   {
      NPCPlayer temp = null;
      float dist = CultimaPanel.SCREEN_SIZE*4;
      int row = p.getRow();
      int col = p.getCol();
      int mi = p.getMapIndex();
      ...
      { 
         ...
         float currDist = Display.findDistance(row, col, target.getRow(), target.getCol());
         if(currDist < dist)
         {
            dist = currDist;
            temp = target;
...
//returns false if they don't
   ...
   public static void findNPCsInSight()
   {
      ...
      CultimaPanel.mannaDrain = false; 
      CultimaPanel.healthDrain = false;  
      ...
      {  //find greatestNPCinSight for knowing spell or show info of most important visible player
         ...
         if(p.getCharIndex()==NPC.MINDDEVOURER || ((p.getCharIndex()==NPC.BEHOLDER || p.getCharIndex()==NPC.LICHE) && p.getNumInfo() > 0)) 
         {  //see if there is a psionic attack against the player
            if(p.getCharIndex()==NPC.MINDDEVOURER || p.getNumInfo()==1)
            {
               CultimaPanel.mannaDrain = true;
            }
            else if(p.getNumInfo()==2)
            {
               CultimaPanel.healthDrain = true;
            }
         }  
      }
...
//returns false if they get away with it
   public static boolean caughtStealingChest()
   {
      ...
      boolean awareHumanNPCInSight = CultimaPanel.awareHumanNPCInSight;
      String loc = CultimaPanel.player.getLocationType();
      if(loc.contains("frogrealm") || loc.contains("pacrealm"))
         return false;
      int distractionBonus = 0;
      boolean monsterAttack = (CultimaPanel.monsterAndCivInView != null);
  ...
  //spawn a ghost/phantom/liche or demon next to us if we are sleeping in a haunted house or we are a reputed swine player (demon swine challenge)
   public static void spawnGhost(byte monsterType)
   {     //make sure there already isn't a monster there
      int row = CultimaPanel.player.getRow();
      int col = CultimaPanel.player.getCol();
      int mi = CultimaPanel.player.getMapIndex();
      for(int r=row-1; r<=row+1; r++)
         for(int c=col-1; c<=col+1; c++)
         {
            ...
            if(temp != null && (temp.getCharIndex()==NPC.DEMON || temp.getCharIndex()==NPC.GHOST || temp.getCharIndex()==NPC.PHANTOM || temp.getCharIndex()==NPC.LICHE))
               return;
         }
      String locType = CultimaPanel.player.getLocationType();
      byte[][] currMap = CultimaPanel.map.get(mi);
      ...
      if(locs.size() > 0)
      {
         ...
         if(monsterType == NPC.DEMON)
         {
            CultimaPanel.flashColor = java.awt.Color.red;
         }
         else
         {
            CultimaPanel.flashColor = java.awt.Color.white;
         }
         Sound.thunder();
         CultimaPanel.flashFrame = CultimaPanel.numFrames;
         CultimaPanel.flashRadius = CultimaPanel.SCREEN_SIZE;
         CultimaPanel.player.setCamping(false);
         NPCPlayer monster = new NPCPlayer(monsterType, monsterRow, monsterCol, mi, locType);
         if(monsterType == NPC.DEMON)
         {
...
//see if a Demon visits us while sleeping to play Swine, or a ghost/phantom/liche if we are sleeping in or next to a haunted house
   public static void demonVisit()
   {
      boolean inHauntedHouse = CultimaPanel.player.inHauntedHouse();
      int DEMON_VISIT_PROB = Math.max(5, Math.min(25, Player.stats[Player.SWINE_GAMES_WON]));
      if (inHauntedHouse)
      {
         DEMON_VISIT_PROB = Math.max(50,DEMON_VISIT_PROB);
      }
      int mi = CultimaPanel.player.getMapIndex();
      if(!inHauntedHouse && (mi==0 || Player.stats[Player.SWINE_GAMES_WON]==0 || !CultimaPanel.player.isCamping()))
         return;
      if(CultimaPanel.time <= 22)      //demons only appear at midnight when it is stormy
         return;
      if(CultimaPanel.weather < 2 && !inHauntedHouse)
      {
         CultimaPanel.time = 0;
      ...
      if(roll >= DEMON_VISIT_PROB)
      {
         ...
         if(CultimaPanel.days >= 60)
            Achievement.earnAchievement(Achievement.ONE_ROUND_DOWN);  
         if(inHauntedHouse && Math.random() < 0.666)
         {  //spawn a ghost or phantom next to us
            byte monsterType = NPC.GHOST;
            if(Math.random() < 0.5)
            {
               monsterType = NPC.PHANTOM;
            }
            else if(Math.random() < 0.01)
            {
               monsterType = NPC.LICHE;
            }
            spawnGhost(monsterType);  
         }   
         return; 
      }
      //make sure there already isn't a demon there
      spawnGhost(NPC.DEMON);
      CultimaPanel.time = 0;
      CultimaPanel.isNight = (CultimaPanel.time >=20 || CultimaPanel.time <= 6);
      CultimaPanel.days++;
      if(CultimaPanel.days >= 60)
...
//shifted one space in the direction of DIR, for exiting ships
 ...
 //finds an escort mission for an npc with a certain name
   ...
   public static float checkDistanceToEscortee(NPCPlayer p)
   {
      float dist = -1.0f;
   ...
   public static boolean closeToWisp()
   {
      ...
      for(NPCPlayer p:CultimaPanel.worldMonsters)
      {
         if(p.getCharIndex()==NPC.WILLOWISP)
         {
            float distToWisp = (Display.findDistance(p.getRow(), p.getCol(), CultimaPanel.player.getRow(), CultimaPanel.player.getCol()));
            if(distToWisp  <= 4 )
            {
...
//return the distance between targ and your dog if you have one
   public static float distToDog(NPCPlayer targ)
   {
      if(!CultimaPanel.player.getDogName().equals("none"))
      {
         for(NPCPlayer p:CultimaPanel.worldMonsters)
...
//      returns null if it can not be placed
  ...
  //      returns null if there is no ship to grapple with
   public static NPCPlayer shipAlignedForGrapple()
   {
      ...
      if(npc.hasMet())
         return null;                                         
      float dist = Display.findDistance(p.getRow(), p.getCol(), npc.getRow(), npc.getCol());
   ...
   public static void changeWeather()
   {
      ...
      float rainbowProb = 0.25f, fogProb = 0.4f;
      if(Display.isSummer())
         rainbowProb *= 2.0f;
   ...
   public static String locationInfo()
   {
      ...
      else if(locType.equals("frogrealm") )
         return ("Across the daunting lanes of Frog-Realm...");
  ...
  //post: sends a message about the type of terrain the player is moving over
   ...
   public static void movePlatforms()
   {
      int mi = CultimaPanel.player.getMapIndex();
      byte[][]currMap = (CultimaPanel.map.get(mi));
      ArrayList<Terrain>liquids = new ArrayList();
      HashMap<Coord, String> plats = new HashMap<Coord, String>();
      Terrain currSpot =  CultimaPanel.allTerrain.get(Math.abs(currMap[CultimaPanel.player.getRow()][CultimaPanel.player.getCol()]));
      if(currSpot.isMoving())
      {  //if the player is on a moving plat, move them with it.  If moving off the map, kill the player
         String dir = currSpot.getPlatformDirection();
         if(dir.contains("EAST"))
         {
            if(CultimaPanel.player.getCol()==currMap[0].length-1)
            {
               CultimaPanel.player.damage(CultimaPanel.player.getBody());
            }
            else
            {
               CultimaPanel.player.setCol(CultimaPanel.player.getCol()+1);  
            }
         }
         else if(dir.contains("WEST"))
         {
            if(CultimaPanel.player.getCol()==0)
            {
               CultimaPanel.player.damage(CultimaPanel.player.getBody());
            }
            else
      ...
      int startRow=0, numRows = currMap.length;
      int startCol=0, numCols = currMap[0].length;
      if(!CultimaPanel.player.getLocationType().contains("pacrealm") && !CultimaPanel.player.getLocationType().contains("sewer"))
      {
         startRow = Math.max(0, CultimaPanel.player.getRow() - (CultimaPanel.SCREEN_SIZE/2) + 1);
         startCol = Math.max(0, CultimaPanel.player.getCol() - (CultimaPanel.SCREEN_SIZE/2) + 1);
         numRows = Math.min(currMap.length, CultimaPanel.player.getRow() + (CultimaPanel.SCREEN_SIZE/2) + 1);
         numCols = Math.min(currMap[0].length, CultimaPanel.player.getCol() + (CultimaPanel.SCREEN_SIZE/2) + 1);
      }
      for(int r=startRow; r<numRows; r++)
      {
         for(int c=startCol; c<numCols; c++)
         {  //in this row, note if there are any plats and record their positions and which direction they are in a map.  also record the liquid type
            //replace the plats in those positions with the recorded liquid
            //for each recorded plat, advance the Coord values depending on their direction
            //then copy the platforms back to the array from the recorded positions
            //account for wrap around or changing directions
            if(LocationBuilder.outOfBounds(currMap, r, c))
               continue;
            ...
            {
               String dir = current.getPlatformDirection();
               plats.put(new Coord(r, c), dir);
            }
         }
      }
      if(CultimaPanel.player.getLocationType().contains("sewer"))
      {
         liquids.clear();
         liquids.add(TerrainBuilder.getTerrainWithName("TER_I_$Deep_water"));
         liquids.add(TerrainBuilder.getTerrainWithName("TER_I_$Rapid_water"));
      }
      else if(CultimaPanel.player.getLocationType().contains("underworld"))
      {
         liquids.clear();
         liquids.add(TerrainBuilder.getTerrainWithName("TER_I_L_3_$Lava"));
      }
      HashMap<Coord, String> toPlats = new HashMap<Coord, String>();
      for(Coord coord: plats.keySet())
      {
         String dir = plats.get(coord);
         int r = coord.getRow();
         int c = coord.getCol();
         int nextRow = r;
         int nextCol = c;
         ...
         else if(dir.contains("WEST"))
         {
            ...
            if(nextCol < 0)
            {
               nextCol = currMap[0].length-1;
            }
         }
         Terrain nextSpot =  CultimaPanel.allTerrain.get(Math.abs(currMap[nextRow][nextCol]));
         if(nextSpot.isLiquid() || nextSpot.isMoving() || nextSpot.getName().contains("_D_"))
         {
            toPlats.put(new Coord(nextRow,nextCol), dir);
         }
         else
         {
            currMap[r][c] = nextSpot.getValue();
            CultimaPanel.player.setCurrentTerrain((byte)(0));
         }
      }
      for(Coord coord: plats.keySet())
      {
         ...
         if(LocationBuilder.outOfBounds(currMap, r, c))
            continue;
         if(CultimaPanel.allTerrain.get(Math.abs(currMap[r][c])).isMoving())
         {
            currMap[r][c] = getRandomFrom(liquids).getValue();
         }
      }
      for(Coord coord: toPlats.keySet())
      {
         String dir = toPlats.get(coord);
         int r = coord.getRow();
         int c = coord.getCol();
         Terrain toPlace = TerrainBuilder.getTerrainWithName("INR_L_2_$EAST_moving_platform");
         if(dir.contains("WEST"))
         {
            toPlace = TerrainBuilder.getTerrainWithName("INR_L_2_$WEST_moving_platform");
         }
         currMap[r][c] = toPlace.getValue();
      }
   }
   
   //given two coordinates in x/y pixel space representing the starting point and ending point of a lightning bolt,
   //this populates the ArrayList<Coord>lightningNodes where start is the first, end is the last
   //and all of the Coords inbetween will be the points that the lightning arcs to in going from the first to last
   public static void connectLightning(Coord start, Coord end)
   {
      CultimaPanel.lightningFrame = CultimaPanel.numFrames;
      CultimaPanel.lightningNodes.add(start);
      float dist = Display.distanceSimple(start.getRow(), start.getCol(), end.getRow(), end.getCol());
      int rise = (int)Math.round(end.getCol() - start.getCol());
      int run  = (int)Math.round(end.getRow() - start.getRow());
      if(run == 0)
      {
         run = 1;
      }
      int slope = rise/run;
      int numNodes = (int)((end.getCol()-start.getCol())/10);
      //System.out.println(start.getRow()+","+start.getCol()+" "+end.getRow()+","+end.getCol() + " slope:" + slope);
      Coord nextNode = new Coord(start.getRow(), start.getCol());
      for(int i = 0; i<numNodes && dist > 10; i++)
      {  //fill in nodes inbetween
         int nextX = nextNode.getRow() + (10/slope) + rollDie(-5,5);
         int nextY = nextNode.getCol() + 10;
         int xDiff = nextX - end.getRow();
         int yDiff = nextY - end.getCol();
         if(Math.abs(xDiff) > Math.abs(yDiff) && xDiff!=0 && yDiff!=0)
         {
            nextX += (xDiff/Math.abs(xDiff)) * rollDie(Math.abs(xDiff)/Math.abs(yDiff));
         }
         nextNode = new Coord(nextX,nextY);
         CultimaPanel.lightningNodes.add(nextNode);
         dist = Display.distanceSimple(nextNode.getRow(), nextNode.getCol(), end.getRow(), end.getCol());
      }
      CultimaPanel.lightningNodes.add(end);
   }
