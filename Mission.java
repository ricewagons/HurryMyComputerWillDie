...
//**Missions - each defines the index for the missions array
   ...
   public static final byte HAUNTED_DARE = 113;       //given by jester - spend the night in a haunted house
   public static final byte PANGOLIN_RESCUE = 114;    //given by wise - rescue pangolin held in castle or city
   public static final byte PANGOLIN_PROTECT = 115;   //given by guard - protect pangolin from being rescued ("Save" mission)
   public static final byte NUM_MISSIONS = 116;       //CAN ONLY GET AS LARGE AS 127
   ...
   public static final String[][] missions = {
      ...
      //CLEAR_DUNGEON 5
      {"These ADJECTIVE MONSTER_TYPEs have overrun thy dungeon. Purge them for GOLD_REWARD gold coins.",                   "Some ADJECTIVE MONSTER_TYPEs still lurk in thy dungeon. Purge them with haste!",               "Thou has done me great service by clearing thy dungeon. Here are thy GOLD_REWARD coins."},
      //CLEAR_MINE 6
      ...
      //WEREWOLF_TOWN
      {"The Wolfen have been banished to LOCATION_NAME but are ravaging our livestock. GOLD_REWARD gold to purge them!",   "Thee shall have GOLD_REWARD gold to burn down the Werewolf outcasts of LOCATION_NAME!","Our livestock is now safe from the Wolfen freaks. Here is thy GOLD_REWARD reward!"},
      //SLAY_VAMPIRE
      ...
      //KAIJU = 74
      {"The colossal MONSTER_TYPE is approaching the gates! Drive it away from our home before it destroys us all!",       "The massive MONSTER_TYPE is almost upon us! Wilt thou lead it away from our gates?",           "The MONSTER_TYPE has disappeared over the horizon! Huzzah! Thee cheers for PLAYER_NAME!"},
      ...
      //HAUNTED_DARE = 113 given by jester - spend the night in a haunted house
      {"I bet thee won't spend the night in that old gray house! I dare thee! I double-dare thee! I double-dog dare thee!","That old gray house has such a comfortable bed within. Why not spend the night there?",        "You are brave indeed, O'PLAYER_NAME! My dare has been met, and lest I forget, here is your prize! <<slap>>"},
      //PANGOLIN_RESCUE = 114 given by wise - rescue pangolin held in castle or city
      {"A Gold-Pangolin is being held in LOC_NAME to reap wealth from its shedded scales! Liberate it for this ITEM_NAME!","SKARA Brae will punish us for the Gold-Pangolin being held in LOC_NAME! Liberate it!",         "SKARA Brae will reward thee for thy bravery. I shall reward you this precious ITEM_NAME!"},
      //PANGOLIN_PROTECT = 115 given by guard - protect pangolin from being rescued
      {"Those insolent mages have highered the NPC_NAME gang to liberate our Gold-Pangolin! Stop them for GOLD_REWARD scales!", "Lure the NPC_NAME gang away from our Gold-Pangolin and eliminate them!",                  "Our Pangolin is safe! We can continue to reap wealth from its shedded golden scales. Here is thy pay."}
      };
...
//returns the short info that is displayed to the user in the Map
   public String buildInfo()
   {
      ...
      else if(missionType == PANGOLIN_RESCUE && clearLocation.length()>0 && !clearLocation.equals("none"))
      {
         if(targetRow==-1 && targetCol==-1)
            return "("+worldCol+"-"+worldRow+"):Liberate the Pangolin from " + clearLocation;
         return "("+worldCol+"-"+worldRow+"):Liberate the Pangolin at (" + targetCol + "-" + targetRow + ")";      
      }
      else if(missionType == PANGOLIN_PROTECT && clearLocation.length()>0 && !targetHolder.equals("none"))
      {
         return "("+worldCol+"-"+worldRow+"):Keep the "+targetHolder+" gang from rescuing the Gold-Pangolin";
      }
      else if(missionType == HAUNTED_DARE)
      {
         return "("+worldCol+"-"+worldRow+"):Surivive the night sleeping in the old gray house";
      }
      ...
      {
         if(targetRow==-1 && targetCol==-1)
            return "("+worldCol+"-"+worldRow+"):"+type+" "+Display.provinceName(clearLocation);
         else if(targetRow!=-1 && targetCol!=-1)
            return "("+worldCol+"-"+worldRow+"):"+type+" "+Display.provinceName(clearLocation) + " at (" + targetCol + "-" + targetRow + ")";
      }
      ...
      {
         if(targetRow==-1 && targetCol==-1)
            return "("+worldCol+"-"+worldRow+"):contend in "+Display.provinceName(clearLocation);
         else if(targetRow!=-1 && targetCol!=-1)
            return "("+worldCol+"-"+worldRow+"):contend in "+Display.provinceName(clearLocation) + " at (" + targetCol + "-" + targetRow + ")";
      }
      ...
      else if(type.equals("Slay") && target!=null)
      {
         if(clearLocation.length()>0 && !targetHolder.equals("none") && targetRow>=0 && targetCol>=0)
            return "("+worldCol+"-"+worldRow+"):"+type+" "+targetHolder + " at " + Display.provinceName(clearLocation)+"(" + targetCol + "-" + targetRow + ")";
         if(clearLocation.length()>0 && !targetHolder.equals("none"))
            return "("+worldCol+"-"+worldRow+"):"+type+" "+targetHolder + " at " + Display.provinceName(clearLocation);
   ...
   {
      if(missionType == PANGOLIN_RESCUE)
      {
         Location missLoc = CultimaPanel.getLocation(this.getClearLocation());
         return (LocationBuilder.countMonsterType(missLoc, NPC.PANGOLIN)==0);
      }
      else if((missionType == COLLECT_FLORETS1) && CultimaPanel.player.fiveFlowersOfEach())
      {
         return true;
      }
      ...
      else if((this.getType().contains("Save") || missionType == PANGOLIN_PROTECT) && this.getClearLocation()!=null)
      {
         ...
         { //create a new mission
            ...
            if(missionType.equals("Arena"))
            {
               ...
               if(loc!=null)
               {
                  ...
                  String locName = loc.getName();//Display.provinceName(loc.getName());
   ...
   public static String guardMission(NPCPlayer selected)
   {
      ...
      for(int cr = 5; cr<currMap.length-5; cr++)
         ...
         {
            if(currLoc.contains("city") || currLoc.contains("fort") || currLoc.contains("village"))
            {
      ...
      if(currMission!=null)   //see if we finished the mission
      {
         ...
         else if(currMission.getMissionType() == PANGOLIN_PROTECT && currMission.getClearLocation()!=null)
         {
         ...
         } //end of checking save mission finish
      ...
      if(selected.getNumInfo() <= 0)
         ...
         if(currMission!=null && currMission.getState() == Mission.IN_PROGRESS)
            ...
            { //create a new mission
               ...
               boolean hasPangolinHere = (LocationBuilder.countMonsterType(currentLoc, NPC.PANGOLIN) > 0);
               String thisLocType = selected.getLocationType().toLowerCase();
               if(CultimaPanel.missionsGiven[PANGOLIN_PROTECT]==0 && hasPangolinHere && spawnCoords.size()>0) 
                  missTemp.add("Protect");
               ...
               if(missTemp.size()==0)  
               {   
                  if(hasPangolinHere && spawnCoords.size()>0)
                  {
                     missTemp.add("Protect");
                  }
                  if(hasSewer)
               ...
               //***TESTING**** if(hasPangolinHere && spawnCoords.size()>0)  missionType = "Protect"; 
               ...
               if(missionType.equals("Fix"))
               {
                  ...
                  else
                  {
                     ...
                     else
                     {
                        ...
                        if(Math.random() < 0.5 && clogR+1 < sewerMap.length && (CultimaPanel.allTerrain.get(Math.abs(sewerMap[clogR+1][clogC])).getName().toLowerCase().contains("bog") || CultimaPanel.allTerrain.get(Math.abs(sewerMap[clogR+1][clogC])).getName().toLowerCase().contains("shallow")))
                        {
                           sewerMap[clogR+1][clogC] = lumber;
                        }
                        if(Math.random() < 0.5 && clogR-1 >= 0 && (CultimaPanel.allTerrain.get(Math.abs(sewerMap[clogR-1][clogC])).getName().toLowerCase().contains("bog") || CultimaPanel.allTerrain.get(Math.abs(sewerMap[clogR-1][clogC])).getName().toLowerCase().contains("shallow")))
                        {
                           sewerMap[clogR-1][clogC] = lumber;
                        }
                        if(Math.random() < 0.5 && clogC+1 < sewerMap[0].length && (CultimaPanel.allTerrain.get(Math.abs(sewerMap[clogR][clogC+1])).getName().toLowerCase().contains("bog") || CultimaPanel.allTerrain.get(Math.abs(sewerMap[clogR][clogC+1])).getName().toLowerCase().contains("shallow")))
                        {
                           sewerMap[clogR][clogC+1] = lumber;
                        }
                        if(Math.random() < 0.5 && clogC-1 >= 0 && (CultimaPanel.allTerrain.get(Math.abs(sewerMap[clogR][clogC-1])).getName().toLowerCase().contains("bog") || CultimaPanel.allTerrain.get(Math.abs(sewerMap[clogR][clogC-1])).getName().toLowerCase().contains("shallow")))
                        {
                           sewerMap[clogR][clogC-1] = lumber;
                        }
               ...
               else if(missionType.equals("Protect") && spawnCoords.size() > 0)
               { 
                  String targetHolder = Utilities.nameGenerator("name");
                  String type = "Protect";
                  String [] temp = missions[PANGOLIN_PROTECT];
                  String [] story = new String[3];
                  story[0] = new String(temp[0]);
                  story[1] = new String(temp[1]);
                  story[2] = new String(temp[2]);
               ...
               //***********************Mission(String t, String[]story, int wRow, int wCol, int gold,   Item iw,  String loc,     Item targ, String targHolder,  int targR, int targC, byte missionType)
                  Mission saveMission = new Mission(type,     story,       startRow, startCol, goldReward, null, currentLoc.getName(), null,      targetHolder,      mRow,      mCol, PANGOLIN_PROTECT, -1);
                  selected.setMissionInfo(saveMission.getInfo());
                  CultimaPanel.missionStack.add(saveMission);
                  if(CultimaPanel.missionsGiven[PANGOLIN_PROTECT]==0)
                     CultimaPanel.missionsGiven[PANGOLIN_PROTECT]++;
                  FileManager.saveProgress();   
               ...
               else if(missionType.equals("Purge"))
               {
                  ...
                  float distToCastle = Display.wrapDistance(closeCastle.getRow(), closeCastle.getCol(), CultimaPanel.player.getWorldRow(), CultimaPanel.player.getWorldCol());           
                  //find next closest town
                  String []locTypes2 = {"city","fortress","port","village"};
                  Location closeCity = TerrainBuilder.closeLocationAndBuild(CultimaPanel.allLocations, locTypes2);
                  ArrayList<NPCPlayer>  cityNPCs = CultimaPanel.civilians.get(closeCity.getMapIndex());
                  float distToCity = Display.wrapDistance(closeCity.getRow(), closeCity.getCol(), CultimaPanel.player.getWorldRow(), CultimaPanel.player.getWorldCol());           
   ...
   public static String royalGuardMission(NPCPlayer selected)
   {
      ...
      else
      {
         ...
         //TESTING randMission = CASTLE_BEAST;
         ...
         if(randMission == TRAIN_HORSE)
         {
            ...
            story[2] = new String(temp[2]);
            int goldReward = 100;
            float priceShift = (float)((Math.random()*51) - 25)/100.0f;  //-.25 to +.25
         ...
         {  //add a beast into loc
            byte monsterType = NPC.randomDungeonMonsterKing();
            //TESTING  monsterType = NPC.GORGON;
            
            int mi = loc.getMapIndex();            
            byte[][]currMap = (CultimaPanel.map.get(mi));
            int r = (int)(Math.random()*currMap.length);
            int c = (int)(Math.random()*currMap[0].length);            
            ArrayList<Coord>spawnCoords = new ArrayList<Coord>();
            ...
            if(spawnCoords.size() > 0)
            {
               ...
               r = spawn.getRow();
               c = spawn.getCol();
               int numCorpses = Utilities.rollDie(2, 8);
               for(int i=0; i<numCorpses && spawnCoords.size() > 0; i++)
               {
                  Coord corpseSpawn = spawnCoords.remove((int)(Math.random()*spawnCoords.size()));
                  int corpseRow = corpseSpawn.getRow();
                  int corpseCol = corpseSpawn.getCol();
                  if(!LocationBuilder.outOfBounds(currMap, corpseRow, corpseCol) && !LocationBuilder.isImpassable(currMap, corpseRow, corpseCol) && Utilities.getCorpseAt(corpseRow, corpseCol, mi)==null)
                  {
                     byte charType = NPC.randomBattlefieldWarrior();
                     NPCPlayer deadGuy = new NPCPlayer(charType, corpseRow, corpseCol, mi, corpseRow, corpseCol, "dungeon");                
                     if(monsterType == NPC.GORGON)
                     {     //make the corpse turned to stone
                        deadGuy.setNumInfo(-999);
                        CultimaPanel.civilians.get(mi).add(deadGuy);
                     }
                     else
                     {
                        CultimaPanel.corpses.get(mi).add(deadGuy.getCorpse()); 
                     }
                  }
               }     
            } 
            ...
            NPCPlayer targNPC = new NPCPlayer(monsterType, r, c, mi, "dungeon");
            String target = NPC.characterDescription(monsterType)+"-head";
            int goldReward = 250;
            targNPC.addItem(new Item(target, goldReward));
            targNPC.setMoveType(NPC.CHASE);
            CultimaPanel.civilians.get(mi).add(targNPC);
   ...
   public static String swordMission(NPCPlayer selected)
   {
      ...
      {  //CLEAR_MINE, FIND_WEAPON, TREASURE_MAP
         ...
         ArrayList<Byte> missionTypes = new ArrayList<Byte>();
         if(waterSpots.size() >= 5 && Utilities.dockSpots().size() == 0 && CultimaPanel.missionsGiven[BUILD_PIER]==0)
         ...
         if(missionTypes.size()==0)
         {
            ...
            if(hasSewer)
               missionTypes.add(Mission.SEWER_SLAY);
            missionTypes.add(Mission.HEAL_WARRIOR);
            if(waterSpots.size() >= 5 && Utilities.dockSpots().size() == 0)
               missionTypes.add(Mission.BUILD_PIER);
            missionTypes.add(Mission.CLEAR_MINE);
            missionTypes.add(Mission.FIND_WEAPON);
            if(kaijuSpawn!=null)
        ...
        //try to build a good treasure map
         ...
         else if(randMission == SEWER_SLAY)
         {
            ...
            {  //add a beast into loc   
               int mapIndex = sewerLoc.getMapIndex();                        
               byte[][]currMap = (CultimaPanel.map.get(mapIndex));
               int r = (int)(Math.random()*currMap.length);
               int c = (int)(Math.random()*currMap[0].length);
               ...
               else
               {
                  ...
                  r = spawn.getRow();
                  c = spawn.getCol();
                  NPCPlayer targNPC = new NPCPlayer(monsterType, r, c, mapIndex, "sewer");
                  int numCorpses = Utilities.rollDie(1, 3);
                  for(int i=0; i<numCorpses && spawnCoords.size() > 0; i++)
                  {
                     Coord corpseSpawn = spawnCoords.remove((int)(Math.random()*spawnCoords.size()));
                     int corpseRow = corpseSpawn.getRow();
                     int corpseCol = corpseSpawn.getCol();
                     if(!LocationBuilder.outOfBounds(currMap, corpseRow, corpseCol) && !LocationBuilder.isImpassable(currMap, corpseRow, corpseCol) && Utilities.getCorpseAt(corpseRow, corpseCol, mapIndex)==null)
                     {
                        byte charType = NPC.randomBattlefieldWarrior();
                        NPCPlayer deadGuy = new NPCPlayer(charType, corpseRow, corpseCol, mapIndex, corpseRow, corpseCol, "sewer");                
                        CultimaPanel.corpses.get(mapIndex).add(deadGuy.getCorpse()); 
                     }
                  }     
         ...
         else if(randMission==KAIJU)
         {
            ...
            //***TESTING*** monsterIndex = NPC.TROGDOR;
   ...
   public static String cityMission(NPCPlayer selected)
   {
     ...
     //***
      ...
      //CITY_REVENGE, FIND_PERSON, MESSAGE, CLEAR_BEGGERS, TRAIN_DOG, CLEAR_BANDITS, FIND_SERPENT_EGGS, FIRE_FIGHTER, FIND_GEM, FIND_SCALES
         ...
         //***TESTING***  randMission = Mission.BEAT_SWINE_PLAYER;    
         ...
         { 
            ...
            float[]directionDist = new float[4];
   ...
   {//make sure ceremony, marry missions are only in cities/port towns
      ...
      if(currMission!=null)   //see if we finished the mission
      {
         if(currMission.getMissionType() == PANGOLIN_RESCUE)
         {
            Location missLoc = CultimaPanel.getLocation(currMission.getClearLocation());
            if(LocationBuilder.countMonsterType(missLoc, NPC.PANGOLIN)==0)
            {
               finishMission(currMission, selected, 10);
            }
         }
         else if(currMission.getMissionType() == COLLECT_FLORETS2 && CultimaPanel.player.enoughFlowers()) 
         {
      ...
      {  
         ...
         boolean hasPangolinHere = (LocationBuilder.countMonsterType(currentLoc, NPC.PANGOLIN) > 0);
         boolean hasPangolinCastle = false;
         ...
         else
         {
            ...
            if(closeCastle != null)
            {
               hasPangolinCastle = (LocationBuilder.countMonsterType(closeCastle, NPC.PANGOLIN) > 0);
            }
         }
         ...
         float distToCaerbannog = Float.MAX_VALUE;
         ...
         if(CultimaPanel.missionsGiven[PANGOLIN_RESCUE]==0 && (hasPangolinHere || hasPangolinCastle)) 
            missionTypes.add(PANGOLIN_RESCUE);
         ...
         if(missionTypes.size()==0)    //we have seen all the mission types before, so pick a random one
         {
            ...
            if(hasGrayTiles) 
               missionTypes.add(CLOSE_HELLMOUTH);
            if(hasPangolinHere || hasPangolinCastle)
               missionTypes.add(PANGOLIN_RESCUE);
         ...
         //***TESTING*** if(hasPangolinHere || hasPangolinCastle)  randMission = PANGOLIN_RESCUE;
         
         if(randMission == PANGOLIN_RESCUE)
         {  
            type = "Rescue";
            String [] temp = missions[randMission];
            String [] story = new String[3];
            ...
            for(int i=0; i<story.length; i++)
            {
               story[i]=story[i].replace("ITEM_NAME", randMagicItem.getName());
               story[i]=story[i].replace("LOC_NAME", missLoc.getName());
            }
            int startRow = CultimaPanel.player.getWorldRow();
            int startCol = CultimaPanel.player.getWorldCol();
            int mRow = -1, mCol = -1;
            ...
            Mission clearMission = new Mission(type, story, startRow, startCol, goldReward, randMagicItem, missLoc.getName(), null, "none", mRow, mCol, PANGOLIN_RESCUE, -1);
            selected.setMissionInfo(clearMission.getInfo());
            CultimaPanel.missionStack.add(clearMission);
            if(CultimaPanel.missionsGiven[PANGOLIN_RESCUE]==0)
               CultimaPanel.missionsGiven[PANGOLIN_RESCUE]++;
            response = clearMission.getStartStory();
            FileManager.saveProgress();
         }
         else if(randMission == COLLECT_FLORETS2)
         {  
            type = "Harvest";
            NPCPlayer targNPC = selected;       
            if(targNPC!=null)
   ...
   public static String jesterMission(NPCPlayer selected)
   {
      ...
      Location loc = null;
      Item target = null;
      Location thisLoc = CultimaPanel.allLocations.get(selected.getMapIndex());
      String type = "";
      String missionInfo = selected.getMissionInfo();
      Mission currMission = null;
      String [] magicDaggers = {"Poison-dagger","Souldagger","Magmadagger","Frostdagger","Banedagger"};
      boolean hauntedHouseWithBed = false;
      if(!selected.getLocationType().contains("castle") && !selected.getLocationType().contains("tower"))
      {
         hauntedHouseWithBed = TerrainBuilder.hasHauntedHouseWithBed(thisLoc);
      }
      int currMissionIndex = -1;
      for(int i=0; i<CultimaPanel.missionStack.size(); i++)
      {
      ...
      if(currMission!=null)   //see if we finished the mission
      {
         ...
         else if(currMission.getType().contains("Find") && CultimaPanel.player.hasItem(targetName))
         {
            CultimaPanel.player.removeItem(targetName);
            selected.setMoveTypeTemp(NPC.RUN);
            int reputationPoints = 5;
            finishMission(currMission, selected, reputationPoints);
      ...
      {  //STEAL_PICK, STEAL_BOOK, STEAL_DICE, STEAL_DAGGERS or DESTROY_STATUE
         ...
         if(hauntedHouseWithBed && CultimaPanel.missionsGiven[HAUNTED_DARE]==0)
            missionTypes.add(HAUNTED_DARE);
         ...
         //***TESTING*** if(hauntedHouseWithBed) randMission = HAUNTED_DARE;
         ...
         else if(randMission == HAUNTED_DARE)
         {
            loc = CultimaPanel.allLocations.get(CultimaPanel.player.getMapIndex());
            if(loc!=null)
            {
               ...
               story[2] = new String(temp[2]);
               int goldReward = 0;
               String targName = "haunted-night-journal-entry";                       
               String locName = Display.provinceName(loc.getName());
               for(int i=0; i<story.length; i++)
               {
                  story[i]=story[i].replace("PLAYER_NAME", CultimaPanel.player.getShortName());
               ...
               int startCol = CultimaPanel.player.getWorldCol();
               int mRow = -1, mCol = -1;
               Mission getMission = new Mission(type, story, startRow, startCol, goldReward, null, locName, new Item(targName, goldReward), "", mRow, mCol, HAUNTED_DARE, -1);
               selected.setMissionInfo(getMission.getInfo());
               CultimaPanel.missionStack.add(getMission);
               if(CultimaPanel.missionsGiven[HAUNTED_DARE]==0)
                  CultimaPanel.missionsGiven[HAUNTED_DARE]++;
               FileManager.saveProgress();
               response = getMission.getStartStory();
            }
         }  
