...
//STAFF SPELLS
 ...
 //MAGIC ITEMS  
   ...
   public static final byte RESTING = 0;
   public static final byte MAPPING = 1;
   public static final byte TRAPPING = 2;
   public static final byte HOME = 3;
   public static final byte DOG = 4;
   public static final byte HORSE = 5;
   public static final byte UNICORN = 6;
   public static final byte SHIP = 7;  
   public static final byte VAMPIRE = 8;
   public static final byte WEREWOLF = 9;
   public static final byte SWINE = 10;
   public static final byte JEM = 11;
   public static final byte NUM_TOPICS = 12;
   //topics for puzzles
   ...
   public static final byte UNDERWORLD = 14;
   public static final byte NUM_LOCATIONS = 15;
   ...
   private static final ImageIcon mimic = new ImageIcon("images/characters/monsters/mimic.gif");
   private static final ImageIcon pangolin = new ImageIcon("images/characters/animals/pangolin.gif");
   ...
   { 
      ...
      { 
         ...
         readable = new Font(fontName, Font.PLAIN, CultimaPanel.SIZE);
         readableP = new Font(fontName, Font.PLAIN, CultimaPanel.SIZE);
         handWriten = new Font(fontName, Font.PLAIN, CultimaPanel.SIZE);
         oldEngl = new Font(fontName, Font.PLAIN, CultimaPanel.SIZE);
         oldEnglBold = new Font(fontName, Font.PLAIN, CultimaPanel.SIZE);
         oldEnglTitle = new Font(fontName, Font.BOLD, CultimaPanel.SIZE*4);
      }
      else
      {
         readable = new Font("Serif", Font.BOLD | Font.ITALIC, CultimaPanel.SIZE);
         readableP = new Font("Serif", Font.PLAIN | Font.ITALIC, CultimaPanel.SIZE);
         handWriten = new Font("Pristina", Font.BOLD | Font.ITALIC, CultimaPanel.SIZE);
         oldEngl = new Font("Old English Text MT", Font.PLAIN, CultimaPanel.SIZE);
         oldEnglBold = new Font("Old English Text MT", Font.BOLD, CultimaPanel.SIZE);
         oldEnglTitle = new Font("Old English Text MT", Font.BOLD, CultimaPanel.SIZE*4);
      }
   }   
...
//                                                 with an Arraylist of multiple sentences for each monster type
   ...
   //START STORY
      ...
      for(int i=0; i < CultimaPanel.player.getLocationInfo() && i < NPC.locationInfo.length; i++)
      {
         ...
         if(temp.contains("~~"))                            //dialogue we want recorded in the journal ends with "~~"
         {
            ...
            if(parts.length == 2)
            {
               ...
               if(topic.equals("BATTLEFIELD"))
                  ...
                  locationIndex = 13;
               else if(topic.equals("UNDERWORLD"))
                  locationIndex = 14;
   ...
   //TRAINING
      ...
      for(int i=0; i < NPC.restInfo.length; i++)
         trainingInfo[RESTING].add(NPC.restInfo[i]);
    ...
    //STATS
      int num = (Math.min(NPC.tavernTown.length, CultimaPanel.player.getTavernInfo()) +
                 ...
                 Math.min(NPC.locationInfo.length, CultimaPanel.player.getLocationInfo()) +
      ...
      float denom = (NPC.tavernTown.length + NPC.monsters.length + NPC.armsInfo.length + NPC.spellInfo.length 
      ...
      float numRevealed = CultimaPanel.revealedMap;
      int mapSize = ((CultimaPanel.map.get(0)).length * (CultimaPanel.map.get(0))[0].length);
      float percentComplete = numRevealed / mapSize * 100;
...
//after the title of the chapter, we can get 20 lines of info per page
   ...
   {	 
      ...
      { 
         ...
         else if(CultimaPanel.player.getStartStoryIndex() == Player.LUTE)
         {
            temp = new ImageIcon(Player.guardDown[CultimaPanel.player.getStartStoryIndex()][0]);
         } 
      ...
      { 
         ...
         float complete = (((float)(Math.min(NPC.locationInfo.length, CultimaPanel.player.getLocationInfo()))) / NPC.locationInfo.length) * 100;
         g.drawString("Locations: "+(int)(complete)+"%", x, y);
      ...
      {
         g.setFont(oldEnglBold);
         float complete = (((float)(Math.min(NPC.monsters.length, CultimaPanel.player.getMonsterInfo()))) / NPC.monsters.length) * 100;
      ...
      //draw frame to put monster image into   
         ...
         if(topIndex <= NPC.BRIGAND_SWORD)
         {
            ...
            {//draw the monster's image in the right info-bar if you have information on it                                     
               ...
               temp.setDirection(AnimatedEntity.LEFT);
               if(topIndex == NPC.MIMIC)
               {
                  drawPictureInFrame(g, mimic);
               }
               else if(topIndex == NPC.PANGOLIN)
               {
                  drawPictureInFrame(g, pangolin);
               }
               else
               {   
               ...
               else if(topIndex == NPC.SORCERER)
                  g.drawString("Lightning/Fireball: range 5-8", x, y+=SIZE);
               else if(topIndex == NPC.GORGON)
                  g.drawString("Magic bow/Stonecast: range 8-10", x, y+=SIZE);
      ...
      { 
         g.setFont(oldEnglBold);
         float complete = (((float)(Math.min(NPC.armsInfo.length, CultimaPanel.player.getArmsInfo()))) / NPC.armsInfo.length) * 100;
      ...
      { 
         g.setFont(oldEnglBold);
         float complete = (((float)(Math.min(NPC.spellInfo.length, CultimaPanel.player.getSpellIndex()))) / NPC.spellInfo.length) * 100;
      ...
      { 
         g.setFont(oldEnglBold);
         float complete = (((float)(Math.min(NPC.tavernTown.length, CultimaPanel.player.getTavernInfo()))) / NPC.tavernTown.length) * 100;
      ...
      { 
         g.setFont(oldEnglBold);
         float complete = (((float)(Math.min(CultimaPanel.player.getRiddleInfo(),NPC.riddleQst.length))) / NPC.riddleQst.length) * 100;
      ...
      {
         g.setFont(oldEnglBold);
         float numerator = (Math.min(NPC.wellsPuzzleInfo.length, CultimaPanel.player.getWellsPuzzleInfo()))
         ...
         float denom = NPC.wellsPuzzleInfo.length + NPC.doorsPuzzleInfo.length + NPC.towersPuzzleInfo.length;
         float complete = (numerator / denom) * 100;
      ...
      {
         g.setFont(oldEnglBold);
         float numerator = (Math.min(NPC.dogInfo.length, CultimaPanel.player.getDogInfo()) + 
                            Math.min(NPC.horseInfo.length, CultimaPanel.player.getHorseInfo()) + 
                            Math.min(NPC.shipInfo.length, CultimaPanel.player.getShipInfo()) + 
                            Math.min(NPC.vampireInfo.length, CultimaPanel.player.getVampireInfo()) + 
                            Math.min(NPC.werewolfInfo.length, CultimaPanel.player.getWerewolfInfo()) + 
                            Math.min(NPC.mappingInfo.length, CultimaPanel.player.getMappingInfo()) +
                            Math.min(NPC.trappingInfo.length, CultimaPanel.player.getTrappingInfo()));
         float denom = NPC.dogInfo.length + NPC.horseInfo.length + NPC.shipInfo.length + NPC.vampireInfo.length + NPC.werewolfInfo.length + NPC.mappingInfo.length + NPC.trappingInfo.length;
         float complete = (numerator / denom) * 100;
         ...
         for(byte i=CultimaPanel.journalPageIndex; i<trainingInfo.length; i++)
         {
            ...
            if(i==DOG)
               ...
               name = "Camping and Resting Advice";   
            else if(i==SWINE)
      ...
      //draw frame to put image into   
         ...
         if(CultimaPanel.journalPageIndex==HORSE)
            ...
            topIndex = NPC.MAN; 
         else if(CultimaPanel.journalPageIndex==RESTING)
            topIndex = NPC.MAN; 
         ...
         {//draw the monster's image in the right info-bar if you have information on it                                     
            NPCPlayer temp = new NPCPlayer(topIndex, 0, 0, 0, "");
            temp.setDirection(AnimatedEntity.LEFT);
            if(CultimaPanel.journalPageIndex==RESTING)
            {
               ImageIcon campImage = new ImageIcon("images/characters/player/camp1.Gif");
               drawPictureInFrame(g, campImage);
            }
            else if(CultimaPanel.journalPageIndex==HOME)
            {
               ImageIcon campImage = new ImageIcon("images/locations2/hut.Gif");
               drawPictureInFrame(g, campImage);
            }
            else
            {
      ...
      else if(CultimaPanel.journalChapterIndex == STATS)
      {
         ...
         if(CultimaPanel.simpleFonts)
         {
            g.setFont(new Font("Serif", Font.PLAIN, (int)(CultimaPanel.SIZE/1.5)));
         }
         else
         {
            g.setFont(handWriten);
      ...
      { 
         ...
         for(byte ach:CultimaPanel.achievements)
            sum += ach;
         float complete = (sum / CultimaPanel.achievements.length) * 100;
   ...
   public static void learnAllBooks()
   {
      ...
      if(CultimaPanel.player.getLocationInfo() < NPC.locationInfo.length)
      {
         int toAdd = NPC.locationInfo.length;
         for(int i=0; i<toAdd; i++)
            CultimaPanel.player.nextLocationInfo();
      }
