...
{
   //****TESTING*** private static final float missionProb = 3.0f;     //to make it so most NPCs have missions to give so we can test specific mission types
   private static final float missionProb = 0.18f;    //% chance of an npc with a mission
   private static final float werewolfProb = 0.0085f; //% chance of an npc with the werewolf curse
   ...
   private float hitTime, missTime;                  //used to show hit graphic when the NPC gets damaged
   private float attackHitTime, attackMissTime;      //used to show hit graphic when the NPC attacks the player
   ...
   private float attackTime;                         //to use for timing of attacks against the player
...
//ARGS:  NPCPlayer(characterIndex, row, col, mapIndex, homeRow, homeCol, locationType)
   public NPCPlayer(byte index, int r, int c, int mi, int hr, int hc, String loc)
   {
      ...
      Math.random() < 0.1f) && NPC.isCivilian(index) && index!=NPC.CHILD && index!=NPC.KING && index!=NPC.JESTER)    
      {  //make the person from a town different from the one they are found in
         if(CultimaPanel.allDomiciles != null)
         {
            ...
            {      
               ...
               if(Math.random() < 0.25f)   //25% of the time, the town can not be found
   ...
   //people in more populated areas are more likely to have more gold
      ...
      fireResistant = false;
      formerCultist = false;
      if(Math.random() < 0.2f)         //20% of the time, make the potential to have any kind of reputation
...
//basicInfo is in the form: getName()+","+getMight()+","+getMind()+","+getAgility()+","+getBody()+","+getCharIndex()
   ...
   public float getPercentHealth()
   {
      ...
      return ((this.getBody()) * 100.0f) / (maxHealth);
   }

   public void setAttributes(byte index)
   {
      boolean indoors = TerrainBuilder.indoors(this);
      ...
      switch (index)
      {
         ...
         case NPC.CHILD:      
            ...
            if(Math.random() < 0.1f)        //chance will have rations
            {
         ...
         case NPC.GUARD_SPEAR:      
            ...
            if(Math.random() < 0.15f)       //chance will have rations
            {
         ...
         case NPC.JESTER:       
            ...
            else if(Math.random() < 0.05f && this.getReputation() < -100)
         ...
         case NPC.MINDDEVOURER:                  
            setName("Mind-Devourer"); 
            gold = 0;
            reputation = 0;
            moveType = NPC.ROAM;
            origMoveType = NPC.ROAM;
            ...
            armor=Armor.getHide();
            weapon=Weapon.getFangedBite();
            //numInfo for Mind-Devourer equates to its size
            if(Math.random() < 0.7f) 
               numInfo = 1;
            else
               numInfo = 2;
         ...
         case NPC.PANGOLIN:                    
            setName("Gold-Pangolin"); 
            gold = Utilities.rollDie(100,200);
            reputation = Utilities.rollDie(100,200);
            moveType = NPC.ROAM;
            origMoveType = NPC.ROAM;
            walkAndSwim = true;
            armor=Armor.getHide();
         ...
         case NPC.ROLLINGMENACE:                    
            setName("Rolling-menace"); 
            gold = Utilities.rollDie(100,200);
            reputation = 0;
            moveType = Utilities.rollDie(NPC.MARCH_NORTH, NPC.MARCH_WEST);
            origMoveType = Utilities.rollDie(NPC.MARCH_NORTH, NPC.MARCH_WEST);
            walkAndSwim = true;
            armor=Armor.getHide();
         ...
         case NPC.SORCERER:           
            ...
            if(Math.random() < 0.1f)
               ...
               items.add(Item.getMindTome());
            if(Math.random() < 0.25f)         //numInfo is the number of undead they can summon
         ...
         case NPC.GREATSHIP:           
            ...
            if(Math.random() < 0.33f && !CultimaPanel.player.hasItem("holdall"))
         ...
         case NPC.BRIGANDSHIP:            
            ...
            else if(Math.random() < 0.25f && !CultimaPanel.player.hasItem("holdall"))
               items.add(Item.getHoldall());
            break;
         ...
         case NPC.GORGON:          
            ...
            numInfo = 0;
            armor = Armor.getHide();
            weapon = Weapon.getRandomSpecialBow();   
            break;
      
         case NPC.DRAGONKING:   
            if(Math.random() < 0.5f)
            {
               setName("Queen-Firedragon");
               weapon = Weapon.getBurnination();   
            }
            else
            {
               setName("Queen-Lightningdragon");
               weapon = Weapon.getDragonBolt();   
            }       
            gold = 500;
         ...
         case NPC.BARREL:          
            setName("Barrel"); 
            if(Math.random() < 0.95f)       //chance will not have gold
               gold = 0;
            if(Math.random() < 0.15f)       //chance will have rations
            {
            ...
            {
               gold *= 3;
               if(Math.random() < 0.33f)       //extra chance will have rations
               {
                  numRations = Utilities.rollDie(1,15);
                  items.add(new Item("meat", numRations));
               } 
            ...
            {
               if(Math.random() < 0.05f)      //chance will have a potion
         ...
         case NPC.MAZEMOUTH:                
            ...
            armor=Armor.getNone();
            weapon=Weapon.getDeathtouch(); 
            agility = (byte)(CultimaPanel.player.getAgility()*0.9f);
            break;
         ...
         case NPC.TROGDOR: 
            ...
            flyer = true;
            fireResistant = true;
            armor = Armor.getDragonqueenScale();
            weapon = Weapon.getBurnination();   
            items.add(Item.getDragonQueenScales()); 
            walkAndSwim = true;
            for(int i=0; i< 3; i++)
...
//post: adjust the players stats by percent (for battles)
   public void modifyStats(float percent)
   {
   ...
   public boolean addEffect(String eff)
   {
      ...
      {
         if(this.getCharIndex() == NPC.BARREL || this.getCharIndex() == NPC.DEATH || this.getCharIndex() == NPC.MONOLITH  || this.getCharIndex() == NPC.STONE || this.getCharIndex() == NPC.SLIME || this.isStatue() || this.getCharIndex() == NPC.PANGOLIN  || this.getCharIndex() == NPC.ROLLINGMENACE)   //no effects on stone
            return false;
         if(this.getCharIndex() == NPC.GHOST || this.getCharIndex() == NPC.PHANTOM || (this.getCharIndex() == NPC.LANDSHARK && !eff.contains("stun")))
            return false;     
         if((this.getCharIndex() == NPC.DRAGON || this.getCharIndex() == NPC.DRAGONKING || this.getCharIndex() == NPC.DEMON  || this.getCharIndex() == NPC.DEMONKING || this.getCharIndex() == NPC.TROGDOR) && eff.contains("fire"))
            return false;
         if((NPC.isUndead(this.getCharIndex()) || this.getCharIndex() == NPC.DEMON  || this.getCharIndex() == NPC.DEMONKING || this.getCharIndex() == NPC.SNAKE  || this.getCharIndex() == NPC.SNAKEKING || this.getCharIndex() == NPC.TREE  || this.getCharIndex() == NPC.TREEKING || this.getCharIndex() == NPC.TROGDOR || 
            this.getCharIndex() == NPC.GORGON || this.getCharIndex() == NPC.TRIFFID || NPC.isEtheral(this.getCharIndex()) || this.getCharIndex() == NPC.SPIDER  || this.getCharIndex() == NPC.SPIDERKING || NPC.isVampire(this.getCharIndex()) || this.isZombie() || NPC.isShip(this.getCharIndex())) && eff.contains("poison"))
            return false; 
         ...
         if((this.getCharIndex() == NPC.GRABOID || this.getCharIndex() == NPC.SPIDER || NPC.getSize(this.getCharIndex())>=2 || NPC.needsWater(this) || this.getCharIndex() == NPC.MAGMA) && eff.contains("web"))
            return false;  
         if((eff.contains("curse") || eff.contains("control")) && (this.getCharIndex() == NPC.WISE || this.getCharIndex() == NPC.SORCERER || this.getCharIndex() == NPC.BEHOLDER || this.getCharIndex() == NPC.LICHE || this.getCharIndex() == NPC.MINDDEVOURER))
            return false;
   ...
   public boolean isCaged()
   {
      ...
      int r = this.getRow();
      int c = this.getCol();
      boolean isBlockedIn = (LocationBuilder.isImpassableNotWater(currMap, r-1, c) && LocationBuilder.isImpassableNotWater(currMap, r+1, c) && LocationBuilder.isImpassableNotWater(currMap, r, c-1) && LocationBuilder.isImpassableNotWater(currMap, r, c+1));
      if(isBlockedIn)
      {   //make the prisoner face us when they are caged (as if pleading to be set free)
         if(CultimaPanel.player.getCol() > this.getCol())
         {
            this.setDirection(AnimatedEntity.RIGHT);
         }
         else if(CultimaPanel.player.getCol() < this.getCol())
         {
            this.setDirection(AnimatedEntity.LEFT);
         }
         else if(CultimaPanel.player.getRow() > this.getRow())
         {
            this.setDirection(AnimatedEntity.DOWN);
         }
         else if(CultimaPanel.player.getRow() < this.getRow())
   ...
   public void runEffects()
   {
      ...
      if(isTrapped() && Math.random() < 0.25f)
      {
         damage(1, "trap");
      }
      if(hasEffect("poison") && Math.random() < 0.5f)
      {
         int dam = 5;
         damage(dam, "poison");
      }
      ...
      if(hasEffect("stun"))
      {
         ...
         if(this.getCharIndex()==NPC.LANDSHARK)                   //chance the stun wears off
         {
            if(Utilities.rollDie(1500) < this.getLevel())
               removeEffect("stun");
         }
   ...
   public void setAttackHitTime(float ht)
   {
      attackHitTime = ht;
   }

   public float getAttackMissTime()
   {
      return attackMissTime;
   }

   public void setAttackMissTime(float ht)
   {
      attackMissTime = ht;
   }
...
//returns true if NPC has an item that is tied to a Player mission
   ...
   public void damage(int d, String type)
   {
      ...
      if(body <= 0)
      {
         ...
         float size = NPC.getSize(getCharIndex());
         body = 0;
   ...
   { 
      ...
      else if(this.getCharIndex()==NPC.GRABOID && Math.random()<0.66f)   //graboid will have a random weapon, armor or item in its corpse
      {
      ...
      if(this.canSwim())      //corpses in water decompose faster or sink
         decomposeTime = CultimaPanel.days + 1;  
      float size = NPC.getSize(this.getCharIndex());
      if((this.getCharIndex()==NPC.RUSTCREATURE || this.getCharIndex()==NPC.MINDDEVOURER) && this.getNumInfo()>=1 && this.getNumInfo()<=3)
  ...
  //returns true if this is in range to attack player
   ...
   {
      float dist = Display.findDistance(this.getRow(), this.getCol(), CultimaPanel.player.getRow(), CultimaPanel.player.getCol());
   ...
   public void layDownFire(int targRow, int targCol)
   {
      ArrayList<Coord> firePoints = new ArrayList<Coord>();
      if(targCol == this.getCol())
      {
         if(targRow < this.getRow())
         {  //player is north of trogdor
            for(int r = targRow + 1; r <= this.getRow() - 1; r++)
            {
               firePoints.add(new Coord(r, this.getCol()));
            }
         }
         else if(targRow > this.getRow())
         {  //player is south of trogdor
            for(int r = this.getRow() + 1; r <= targRow - 1; r++)
            {
               firePoints.add(new Coord(r, this.getCol()));
            }
         }
      ...
      {
         if(targCol < this.getCol())
         {  //player is left of trogdor
            for(int c = targCol + 1; c <= this.getCol() - 1; c++)
            {
               firePoints.add(new Coord(this.getRow(), c));
            }
         }
         else if(targCol > this.getCol())
         {  //player is right of trogdor
            for(int c = this.getCol() + 1; c <= targCol - 1; c++)
            {
               firePoints.add(new Coord(this.getRow(), c));
            }
         }
      ...
      if(firePoints.size() > 0)
      {
         for(Coord fireSpot: firePoints)
         {
            ...
            int fSizeMin = 25;
            int fSizeMax = 50;
            NPCPlayer fire = new NPCPlayer(NPC.FIRE, r, c, mi, this.getLocationType());
            fire.setBody(Utilities.rollDie(fSizeMin,fSizeMax));
            CultimaPanel.worldMonsters.add(fire);                
         }
         if(this.getCharIndex()==NPC.TROGDOR)
         {
            CultimaPanel.sendMessage("-----");
            Sound.trogdor();
            CultimaPanel.sendMessage("Burnination!!!");
         }
      }
   }

   public void lightningBolt(boolean hit)
   {
      if(this.getWeapon().getName().toLowerCase().contains("lightning"))
      {  //record start and end lightningNodes from player to target
         ...
         int endX = (CultimaPanel.SCREEN_SIZE/2)*CultimaPanel.SIZE;
         int endY = (CultimaPanel.SCREEN_SIZE/2)*CultimaPanel.SIZE;
         if(!hit)
         {
            endX += Utilities.rollDie(-1*CultimaPanel.SIZE*3, CultimaPanel.SIZE*3);
            endY += Utilities.rollDie(-1*CultimaPanel.SIZE*3, CultimaPanel.SIZE*3);
         }
         Coord lightningEnd = new Coord(endX, endY);
  ...
  //attack the player
   public void attack()
   {
      if(System.currentTimeMillis() < CultimaPanel.immuneTime) //a little of time immune from attacks for the moment we enter a location
         return;
      boolean indoors = TerrainBuilder.indoors(this);          //used to see if fire is less potent in the rain, or freeze more potent in the rain
   ...
   //ship that has been boarded by player should not attack   
      if((this.getCharIndex()==NPC.GREATSHIP || this.getCharIndex()==NPC.BRIGANDSHIP))
      {
         ...
         Sound.fireGun(); 
      if(this.getCharIndex()==NPC.SORCERER) 
      {
         if(this.getNumInfo() > 0 && Math.random() < 0.5f)   //maybe summon undead
         {
            ...
            {
               CultimaPanel.sendMessage("-----");
               CultimaPanel.sendMessage("~In Mani Corp!");
         ...
         if(this.getWeapon().getName().toLowerCase().contains("fireball"))
         {
            CultimaPanel.sendMessage("-----");
            CultimaPanel.sendMessage("~Ignem Mittent!");
         }
         else if(this.getWeapon().getName().toLowerCase().contains("lightning"))
         {
            CultimaPanel.sendMessage("-----");
            CultimaPanel.sendMessage("~Vocare Fulgur!");
         }
      }
      ...
      //creatures with ranged attacks switch to melee attack when closer
      if((this.getCharIndex()==NPC.DRAGONKING || this.getCharIndex()==NPC.TROGDOR) && distance==1)
         weapon = Weapon.getCrushingJaws();
      else if((this.getCharIndex()==NPC.DRAGON) && distance==1)
         weapon = Weapon.getMassiveFists();
      else if((this.getCharIndex()==NPC.BEHOLDER || this.getCharIndex()==NPC.LICHE || this.getCharIndex()==NPC.GORGON) && distance==1)
         weapon = Weapon.getHardPoisionBite();      
      ...
      {  //ranged weapon: make hitDie based on difference between player and monster agility
         ...
         if((weapName.contains("burnination") || weapName.contains("dragonfire")) && distance >= 2)
         {  //trogdor attack - lay down a path of fire
            layDownFire(CultimaPanel.player.getRow(), CultimaPanel.player.getCol());
         }
      }
      float bonusTemp = 1.0f;
      if(heightBonus > 1)
         bonusTemp += 0.5f;
      if(meleeAgainstBow)
         bonusTemp += 0.5f;
      if(this.getCharIndex()==NPC.RUSTCREATURE || this.getCharIndex()==NPC.SLIME || this.getCharIndex()==NPC.MINDDEVOURER)
         bonusTemp += 1.0f;  
            
      if((int)(this.getLevel()*bonusTemp) >= hitDie || CultimaPanel.player.isCamping() || !CultimaPanel.player.isOnGuard() || this.getCharIndex()==NPC.MONOLITH  || this.getCharIndex()==NPC.ROLLINGMENACE || this.getLocationType().contains("pacrealm"))
      {                    //they hit player
         if((this.getCharIndex()==NPC.SPIDER || this.getCharIndex()==NPC.SPIDERKING) && !CultimaPanel.player.hasEffect("web"))
         {
            ...
            {
               if((this.getCharIndex()==NPC.SPIDER && Math.random() < 0.5f) || this.getCharIndex()==NPC.SPIDERKING)
               {
         ...
         if(!weapon.getEffect().equals("none"))
         {
            ...
            else
            {
               ...
               {
                  int effectDie = Utilities.rollDie((this.getMind() + CultimaPanel.player.getMind())*2);
                  if(this.getMind() > effectDie)
                  {
         ...
         if(weapon.getName().toLowerCase().contains("burnination"))
         {
            Utilities.explosion(currMap, CultimaPanel.player.getRow(), CultimaPanel.player.getCol(), CultimaPanel.player.getMapIndex(), CultimaPanel.player.getLocationType());
         }
      }
      if(weapon.getName().toLowerCase().contains("lightning"))
      {
         ...
         {
            if((CultimaPanel.player.isOnGuard() && Math.random()<0.75f) || (!CultimaPanel.player.isOnGuard() && Math.random()<0.25f))
            {
               lightningBolt(true);
               float damage = dam;
               if(CultimaPanel.player.isOnGuard())
                  damage *= (Utilities.rollDie(50,100)/100.0f);
               else
                  damage *=  (Utilities.rollDie(25,75)/100.0f);
               Sound.trapSprung();
      ...
      {
         boolean found = false;
         if(this.getCharIndex()==NPC.RUSTCREATURE && !CultimaPanel.player.getLocationType().contains("frogrealm"))
         ...
         if(NPC.isVampire(this.getCharIndex()) && (CultimaPanel.time <8 || CultimaPanel.time > 16) && !CultimaPanel.player.isVampire() && CultimaPanel.player.getBody() < 50 && Math.random() < 0.5f)
         {  //change player into a Vampire
            Utilities.changeToVampire();
            this.setMoveType(NPC.RUN);
            dam = 0;
         ...
         if((this.getCharIndex()==NPC.WEREWOLF) && !CultimaPanel.player.isWerewolf() && CultimaPanel.player.getBody() < 50 && Math.random() < 0.5f)
         {  //change player into a Werewolf
            Utilities.changeToWerewolf();
            this.setMoveType(NPC.RUN);
            dam = 0;
         }
         if(this.getCharIndex()==NPC.MINDDEVOURER)
         {  //devours your mind, son!
            CultimaPanel.sendMessage("-----");
            CultimaPanel.sendMessage("Thy intellect has been devoured!");
            CultimaPanel.player.setMindNoSound(Math.max(3, CultimaPanel.player.getMindRaw()-Utilities.rollDie(1,3)));
            this.setMoveType(NPC.RUN);
            Sound.summonUndead();
         }
         ...
         { 
            ...
            else
            {
               ...
               float save = Math.max(0, (1 - (roll/100.0f)));
               if(this.getCharIndex()!=NPC.FIRE)
                  dam = (int)(dam * save);
            }
         }  
         if((weapon.getName().toLowerCase().contains("fire")) && fireProtection)
         {
            ...
            //extra saving thow with dragon-scale armor against dragon attack
               float save = 1 - (Utilities.rollDie(25,100)/100.0f);
               if(CultimaPanel.player.getActiveSpell("Fireskin")!=null)
                  save = 1 - (Utilities.rollDie(50,100)/100.0f);
               dam = (int)(dam * save);
            }
         }
         ...
         if((this.getCharIndex() == NPC.JESTER  && !this.isZombie()) || (this.getCharIndex() == NPC.GOBLIN && (Math.random() < 0.25f || (!this.hasBeenAttacked() && Math.random()<0.5f))))
         {
            this.setAttackMissTime(CultimaPanel.numFrames + (CultimaPanel.animDelay*3/2));
            if((CultimaPanel.player.getGold()==0 && CultimaPanel.player.getItems().size() == 0) || (CultimaPanel.player.isOnGuard() && Math.random() < 0.75f))
            {
               Sound.miss();        //sound
            }
            else
            {
               if(this.getCharIndex() == NPC.JESTER  && this.getReputationName().equals("Kanvish") && Math.random() < 0.5f)
               {
                  Sound.slap();
                  CultimaPanel.sendMessage("-----");
               ...
               {
                  if(CultimaPanel.player.getGold()==0 || Math.random() < 0.5f) //steal an item
                  {
         ...
         if(this.getCharIndex()==NPC.SORCERER && weapName.contains("lightning"))
         {
            lightningBolt(true);
         }
      }
      ...
      {  //they miss us
         ...
         this.setAttackMissTime(CultimaPanel.numFrames + (CultimaPanel.animDelay*3/2));
         if((hasShield && Math.random() < 0.75f) || (weapon.getRange() == 1 && CultimaPanel.player.getMight() > CultimaPanel.player.getAgility()))
         {
            Sound.parried();
...
//attack another NPCPlayer (maybe with a specific type of weapon if forceWeapon is not null)
   ...
   //ship that has been boarded by player should not attack   
      if((this.getCharIndex()==NPC.GREATSHIP || this.getCharIndex()==NPC.BRIGANDSHIP) && this.hasMet())
         return; 
      if(this.hasEffect("stun") && Math.random() < 0.75f)
         return;  
      ...
      if((weapName.contains("burnination")  || weapName.contains("dragonfire")) && distance >= 2)
      {  //trogdor attack - lay down a path of fire
         layDownFire(target.getRow(), target.getCol());
      }
      ...
      if(hitDie+this.getLevel() > (dodgeDie + target.getLevel()) || this.getCharIndex()==NPC.MONOLITH || this.getCharIndex()==NPC.ROLLINGMENACE || this.getLocationType().contains("pacrealm"))
      {                    //they hit npc
         dam = Utilities.rollDie(weapon.getMinDamage(), weapon.getMaxDamage()) + this.getLevel();
         ...
         if(!weapon.getEffect().equals("none"))
         {
            ...
            else
            {
               if(weapon.getEffect().contains("curse") || weapon.getEffect().contains("control"))
               {
                  int effectDie = Utilities.rollDie((this.getMind() + target.getMind())*2);
                  if(this.getMind() > effectDie)
                     target.addEffect(weapon.getEffect());
               }
               else
         ...
         {
            byte[][]currMap = (CultimaPanel.map.get(this.getMapIndex()));   
            Utilities.explosion(currMap, target.getRow(), target.getCol(), target.getMapIndex(), target.getLocationType());
         }
      }
      if(this.getCharIndex()==NPC.FIRE)
      {
         if(target.getCharIndex()==NPC.DEMON || target.getCharIndex()==NPC.DEMONKING ||
         target.getCharIndex()==NPC.DRAGON || target.getCharIndex()==NPC.DRAGONKING || target.getCharIndex()==NPC.TROGDOR ||
      ...
      if(dam > 0)
      {
         ...
         if(weapon.getName().toLowerCase().contains("bow") || weapon.getName().toLowerCase().contains("caster"))
         {
            if((weapon.getEffect().contains("freeze") || weapon.getEffect().contains("fire")) && Math.random() < 0.5f)
         ...
         //remove target, add to restore, add Corpse
            ...
            {  //kill the rider and leave the horse
               ...
               if(this.getMapIndex()!=0 || Math.random() < 0.5f)
               {
